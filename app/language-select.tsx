...
const LanguageItem = React.memo(({ item, isSelected, onPress, index }: LanguageItemProps) => {
  ...
  return (
    <Animated.View entering={FadeInDown.delay(index * 30).springify()} style={animStyle}>
      <TouchableOpacity
      ...
      >
        ...
        <Text style={[styles.langName, isSelected && styles.langNameSelected]} numberOfLines={1}>
          {item.name}
        </Text>
...
export default function LanguageSelectScreen() {
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: (typeof SUPPORTED_LANGUAGES)[0]; index: number }) => (
      <LanguageItem
        item={item}
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <Animated.View entering={FadeIn.delay(100)} style={styles.header}>
        <View style={styles.globeContainer}>
          <LinearGradient colors={['#58CC02', '#4CAD02']} style={styles.globeBg}>
