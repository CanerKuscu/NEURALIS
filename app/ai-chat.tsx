...
import { StyleSheet, View, Text, TouchableOpacity, StatusBar, TextInput, KeyboardAvoidingView, Platform, FlatList, ActivityIndicator, Image } from 'react-native';
...
export default function AIChatScreen() {
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            setMessages(prev => [...prev.filter(m => m.id !== userMsg.id), { ...userMsg, id: `user-${Date.now()}` }, aiMsg]);
        } catch {
                ...
                <FlatList
                    ...
                    renderItem={({ item: s, index }) => (
                        <Animated.View entering={FadeInDown.delay(index * 60)}>
                            <TouchableOpacity
                                style={[styles.sessionItem, { backgroundColor: theme.background.secondary, borderColor: session?.id === s.id ? '#2ECC71' : 'transparent', borderWidth: session?.id === s.id ? 2 : 0 }]}
                                onPress={() => selectSession(s)}
                            >
                                <View style={{ flex: 1 }}>
                                    <Text style={[styles.sessionTitle, { color: theme.text.primary }]}>{s.title}</Text>
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top, backgroundColor: theme.background.primary }]}>
            ...
            <View style={[styles.chatHeader, { borderBottomColor: theme.border.light }]}>
                ...
                <TouchableOpacity onPress={() => { setSessions(sessions); setMode('history'); }}>
            ...
            <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }} keyboardVerticalOffset={insets.top + 60}>
                <FlatList
                    ...
                    ListFooterComponent={isLoading ? (
                        <View style={styles.loadingRow}>
                            <View style={[styles.loadingBubble, { backgroundColor: theme.background.secondary }]}>
                                <ActivityIndicator size="small" color={theme.primary} />
                                <Text style={[styles.loadingText, { color: theme.text.secondary }]}>Düşünüyor...</Text>
                            </View>
                        </View>
                    ) : null}
                    renderItem={({ item: msg }) => {
                ...
                {visibleMessages.length <= 2 && (
                    <View style={styles.quickPrompts}>
                        <FlatList
                            ...
                            renderItem={({ item: p }) => (
                                <TouchableOpacity style={[styles.quickBtn, { backgroundColor: theme.background.secondary }]} onPress={() => quickPrompt(p.prompt)}>
                                    <Text style={styles.quickEmoji}>{p.emoji}</Text>
                                    <Text style={[styles.quickLabel, { color: theme.text.primary }]}>{i18n.locale === 'tr' ? p.labelTr : p.label}</Text>
                                </TouchableOpacity>
                            )}
                        />
                    </View>
                ...
                <View style={[styles.inputBar, { backgroundColor: theme.background.secondary, paddingBottom: insets.bottom + 8 }]}>
                    ...
                    <TouchableOpacity onPress={sendMessage} disabled={!inputText.trim() || isLoading}>
                        <LinearGradient
                            colors={inputText.trim() ? ['#2ECC71', '#27AE60'] : [theme.border.light, theme.border.light]}
...
const styles = StyleSheet.create({
    ...
    botAvatar: { width: 38, height: 38, borderRadius: 19, justifyContent: 'center', alignItems: 'center' },
    chatHeaderName: { fontSize: 16, fontWeight: '700' },
    chatHeaderStatus: { fontSize: 12, fontWeight: '500' },
    sessionItem: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14 },
    ...
    msgAvatarSmall: { width: 26, height: 26, borderRadius: 13, backgroundColor: '#2ECC7120', justifyContent: 'center', alignItems: 'center' },
    msgBubble: { maxWidth: '80%', padding: 12, borderRadius: 16 },
    msgText: { fontSize: 15, lineHeight: 22 },
    loadingRow: { flexDirection: 'row', marginTop: 4 },
    loadingBubble: { flexDirection: 'row', alignItems: 'center', gap: 8, padding: 12, borderRadius: 16 },
    loadingText: { fontSize: 13 },
    quickPrompts: { paddingVertical: 8 },
    quickBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 14, paddingVertical: 10, borderRadius: 20 },
    ...
    input: { flex: 1, paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, fontSize: 15, maxHeight: 100 },
