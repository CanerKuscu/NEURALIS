...
import type { ExamSession, ExamConfig } from '../src/services/ExamSimulatorService';
import { examSimulatorService, EXAM_CONFIGS } from '../src/services/ExamSimulatorService';
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <Animated.View entering={FadeInDown.delay(index * 100)}>
              <TouchableOpacity
                style={[styles.examCard, { backgroundColor: theme.background.secondary }]}
                onPress={() => startExam(item)}
              >
                <Text style={styles.examEmoji}>{item.emoji}</Text>
                <View style={styles.examInfo}>
                  <Text style={[styles.examTitle, { color: theme.text.primary }]}>
                    {item.label}
                  </Text>
                  <Text style={[styles.examDesc, { color: theme.text.secondary }]}>
                    {item.description}
                  </Text>
    ...
    return (
      ...
      >
        ...
        <Text style={[styles.loadingText, { color: theme.text.secondary }]}>
          {i18n.t('exam.creating_questions')}
        </Text>
      </View>
    );
  }
  ...
  if (screenState === 'result' && session) {
    ...
    return (
      <ScrollView style={[styles.container, { backgroundColor: theme.background.primary }]}>
        <LinearGradient
          colors={accuracy >= 70 ? ['#2ECC71', '#27AE60'] : ['#E74C3C', '#C0392B']}
        ...
        >
          ...
          <Text style={styles.resultSub}>
            {session.score}/{session.questions.length} doğru
          </Text>
        </LinearGradient>
        ...
        <View style={styles.breakdownContainer}>
          ...
          {session.categoryBreakdown
            ...
            .map((b, i) => (
              ...
              >
                <Text style={[styles.breakdownCat, { color: theme.text.primary }]}>
                  {b.category}
                </Text>
                ...
                <Text style={[styles.breakdownScore, { color: theme.text.secondary }]}>
                  {b.correct}/{b.total}
                </Text>
              </Animated.View>
            ))}
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
      ...
      <View style={[styles.topBar, { paddingTop: insets.top + 8 }]}>
        <Text style={[styles.qCounter, { color: theme.text.secondary }]}>
          {currentQ + 1}/{session.questions.length}
        </Text>
        <View style={styles.timerBadge}>
          <Timer size={16} color={timeLeft <= 60 ? '#FF4B4B' : '#2980B9'} />
          <Text style={[styles.timerText, timeLeft <= 60 && { color: '#FF4B4B' }]}>
            {formatTime(timeLeft)}
          </Text>
        </View>
      </View>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
            ...
            <Animated.View key={i} entering={FadeInDown.delay(i * 50)}>
              ...
              >
                <View style={[styles.optionLetter, { borderColor }]}>
                  <Text style={[styles.letterText, { color: theme.text.primary }]}>
        ...
        {isAnswered && question.explanation && (
          <Animated.View
            entering={FadeInDown}
            style={[styles.explanationCard, { backgroundColor: theme.background.secondary }]}
          >
            <Text style={[styles.explanationLabel, { color: '#2980B9' }]}>📖 Açıklama:</Text>
            <Text style={[styles.explanationText, { color: theme.text.primary }]}>
