...
import { ArrowLeft, TrendingUp, TrendingDown, BarChart3, Calendar, Clock, Target, Brain, Zap } from 'lucide-react-native';
import Animated, { FadeInDown } from 'react-native-reanimated';
import { useTheme } from '../src/context/ThemeContext';
import { analyticsService } from '../src/services/AnalyticsService';
import type { WeeklySummary, MonthlySummary, LearningTrend, PerformanceInsight } from '../src/services/AnalyticsService';
            ...
            <ScrollView contentContainerStyle={{ padding: 20, gap: 16, paddingBottom: 100 }}>
                ...
                {insights.map((insight, i) => (
                    <Animated.View key={i} entering={FadeInDown.delay(i * 80)}>
                        <View style={[styles.insightCard, {
                            backgroundColor: insight.type === 'positive' ? '#2ECC7115' : insight.type === 'negative' ? '#E74C3C15' : theme.background.secondary,
                            borderLeftColor: insight.type === 'positive' ? '#2ECC71' : insight.type === 'negative' ? '#E74C3C' : '#F1C40F',
                        }]}>
                            <Text style={styles.insightEmoji}>{insight.emoji}</Text>
                            <Text style={[styles.insightText, { color: theme.text.primary }]}>{insight.textTr}</Text>
                        </View>
                    </Animated.View>
                ))}
                ...
                <View style={[styles.tabRow, { backgroundColor: theme.background.secondary }]}>
                    <TouchableOpacity style={[styles.tab, tab === 'weekly' && styles.tabActive]} onPress={() => setTab('weekly')}>
                        <Text style={[styles.tabText, { color: tab === 'weekly' ? '#FFF' : theme.text.secondary }]}>Haftalık</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={[styles.tab, tab === 'monthly' && styles.tabActive]} onPress={() => setTab('monthly')}>
                        <Text style={[styles.tabText, { color: tab === 'monthly' ? '#FFF' : theme.text.secondary }]}>Aylık</Text>
                    </TouchableOpacity>
                </View>
                    ...
                    <View style={styles.statsGrid}>
                        <View style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                            ...
                            {weekly.improvement !== 0 && (
                                <View style={[styles.changeTag, { backgroundColor: weekly.improvement > 0 ? '#2ECC7120' : '#E74C3C20' }]}>
                                    {weekly.improvement > 0 ? <TrendingUp size={12} color="#2ECC71" /> : <TrendingDown size={12} color="#E74C3C" />}
                                    <Text style={{ color: weekly.improvement > 0 ? '#2ECC71' : '#E74C3C', fontSize: 11, fontWeight: '700' }}>
                        ...
                        <View style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                            ...
                            <Text style={[styles.statNum, { color: theme.text.primary }]}>{weekly.totalLessons}</Text>
                            <Text style={[styles.statLabel, { color: theme.text.secondary }]}>Ders</Text>
                        </View>
                        <View style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                            <Target size={20} color="#2ECC71" />
                            <Text style={[styles.statNum, { color: theme.text.primary }]}>%{weekly.accuracy}</Text>
                            <Text style={[styles.statLabel, { color: theme.text.secondary }]}>Doğruluk</Text>
                        </View>
                        <View style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                            <Clock size={20} color="#3498DB" />
                            <Text style={[styles.statNum, { color: theme.text.primary }]}>{weekly.totalTime}</Text>
                ...
                {tab === 'monthly' && monthly && (
                    <View style={styles.statsGrid}>
                        ...
                        <View style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                            <Brain size={20} color="#9B59B6" />
                            <Text style={[styles.statNum, { color: theme.text.primary }]}>{monthly.totalLessons}</Text>
                            <Text style={[styles.statLabel, { color: theme.text.secondary }]}>Ders</Text>
                        </View>
                        <View style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                            <Calendar size={20} color="#E67E22" />
                            <Text style={[styles.statNum, { color: theme.text.primary }]}>{monthly.activeDays}</Text>
                            <Text style={[styles.statLabel, { color: theme.text.secondary }]}>Aktif Gün</Text>
                        </View>
                        <View style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                            <Target size={20} color="#2ECC71" />
                            <Text style={[styles.statNum, { color: theme.text.primary }]}>%{monthly.accuracy}</Text>
                            <Text style={[styles.statLabel, { color: theme.text.secondary }]}>Doğruluk</Text>
                        </View>
                    </View>
                )}
                ...
                <Animated.View entering={FadeInDown.delay(300)}>
                    <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>Aktivite Haritası</Text>
                    <View style={[styles.heatmapContainer, { backgroundColor: theme.background.secondary }]}>
                        <View style={styles.heatmapGrid}>
                            {heatmap.map((d, i) => (
                                <View key={i} style={[styles.heatCell, {
                                    backgroundColor: isDark ? (d.level === 0 ? 'rgba(255,255,255,0.05)' : heatColors[d.level]) : heatColors[d.level],
                                }]} />
                            ))}
                        </View>
                        <View style={styles.heatLegend}>
                            ...
                            {heatColors.map((c, i) => (
                                <View key={i} style={[styles.heatCell, { backgroundColor: c, width: 14, height: 14 }]} />
                            ))}
                ...
                {monthly && monthly.categoryBreakdown.length > 0 && (
                    <Animated.View entering={FadeInDown.delay(400)}>
                        <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>Kategori Dağılımı</Text>
                        {monthly.categoryBreakdown.map((cat, i) => (
                            <View key={cat.category} style={[styles.catRow, { backgroundColor: theme.background.secondary }]}>
                                <Text style={[styles.catName, { color: theme.text.primary }]}>{cat.category}</Text>
                                <View style={styles.catBar}>
                                    <View style={[styles.catBarFill, { width: `${cat.percentage}%`, backgroundColor: ['#2ECC71', '#3498DB', '#9B59B6', '#E67E22', '#E74C3C'][i % 5] }]} />
                                </View>
                                <Text style={[styles.catPct, { color: theme.text.secondary }]}>%{cat.percentage}</Text>
                            </View>
                        ))}
                    </Animated.View>
                )}
...
const styles = StyleSheet.create({
    ...
    insightCard: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14, borderLeftWidth: 4 },
    ...
    statCard: { width: (width - 52) / 2, padding: 16, borderRadius: 16, alignItems: 'center', gap: 6 },
    statNum: { fontSize: 24, fontWeight: '800' },
    statLabel: { fontSize: 12, fontWeight: '500' },
    changeTag: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 },
    ...
    barWrapper: { height: 80, width: 20, justifyContent: 'flex-end', borderRadius: 10, overflow: 'hidden' },
    ...
    catRow: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, borderRadius: 12, marginTop: 6 },
    catName: { fontSize: 13, fontWeight: '600', width: 80 },
    catBar: { flex: 1, height: 8, backgroundColor: 'rgba(0,0,0,0.05)', borderRadius: 4, overflow: 'hidden' },
