...
import type { LessonSeries, Lesson } from '../../src/types/lessonSeries';
import { LESSON_CATEGORIES } from '../../src/types/lessonSeries';
...
export default function SeriesDetailScreen() {
        ...
        setSeries({
          ...
          progress:
            seriesLessons.length > 0
              ? Math.round((completedCount / seriesLessons.length) * 100)
              : 0,
          tags: [],
        });
      }
  ...
  const getCategoryInfo = (categoryId: string) => {
    return (
      LESSON_CATEGORIES.find((c) => c.id === categoryId) ||
      LESSON_CATEGORIES[LESSON_CATEGORIES.length - 1]
    );
  };
  ...
  if (loading) {
    return (
      <View
        style={[styles.container, styles.centered, { backgroundColor: theme.background.primary }]}
      >
    ...
    <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {lessons.map((lesson, index) => {
          ...
          return (
            <Animated.View key={lesson.id} entering={FadeInRight.delay(index * 100).duration(500)}>
              <TouchableOpacity
                style={[
                  styles.lessonCard,
                  {
              ...
              >
                ...
                <View style={styles.lessonContent}>
                  ...
                  <View style={styles.lessonMeta}>
                    ...
                    <Text style={[styles.lessonMetaText, { color: theme.text.secondary }]}>•</Text>
                ...
                {!isLocked && <ChevronRight size={20} color={theme.text.muted} />}
              </TouchableOpacity>
            </Animated.View>
          );
        })}
        ...
        {series.progress === 100 && (
          ...
          >
            <LinearGradient colors={['#F1C40F', '#E67E22']} style={styles.completionGradient}>
              <Trophy size={40} color="#FFF" />
              <Text style={styles.completionTitle}>Series Completed! 🎉</Text>
              <Text style={styles.completionText}>You've earned {series.earnedXP} XP</Text>
            </LinearGradient>
          </Animated.View>
        )}
      </ScrollView>
