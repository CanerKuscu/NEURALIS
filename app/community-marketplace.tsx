...
import { StyleSheet, View, Text, TouchableOpacity, FlatList, ActivityIndicator, TextInput, Dimensions } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { ArrowLeft, Search, Star, Heart, Download, Users, BookOpen, Filter, TrendingUp } from 'lucide-react-native';
...
export default function CommunityMarketplaceScreen() {
    ...
    const loadCourses = async () => {
        ...
        try {
            ...
            let query = supabase.from('lesson_series')
            ...
            // Get user favorites
            const { data: favs } = await supabase.from('series_favorites')
                .select('series_id')
    ...
    const handleFavorite = async (courseId: string, isFav: boolean) => {
        ...
        if (isFav) {
            await supabase.from('series_favorites').delete().eq('user_id', userId).eq('series_id', courseId);
        } else {
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            {/* Header */}
            <LinearGradient colors={['#E67E22', '#D35400']} style={[styles.header, { paddingTop: insets.top + 8 }]}>
            ...
            <View style={styles.sortRow}>
                {([
                    ...
                    <TouchableOpacity key={s.id}
                        ...
                        onPress={() => { setSortBy(s.id); Haptics.selectionAsync(); }}>
                        <Text style={[styles.sortTabText, sortBy === s.id && styles.sortTabTextActive]}>{s.label}</Text>
                    </TouchableOpacity>
                ))}
            </View>
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <Animated.View entering={FadeInDown.delay(Math.min(index, 8) * 60)}>
                            <TouchableOpacity style={[styles.courseCard, { backgroundColor: theme.background.secondary }]} onPress={() => handleOpen(item.id)}>
                                <View style={styles.courseHeader}>
                                    <Text style={styles.courseEmoji}>{CATEGORY_EMOJI[item.category] || '📚'}</Text>
                                    <View style={styles.courseInfo}>
                                        <Text style={[styles.courseTitle, { color: theme.text.primary }]} numberOfLines={1}>{item.title}</Text>
                                        <Text style={[styles.courseCreator, { color: theme.text.secondary }]}>by {item.creatorName}</Text>
                                    </View>
                                    <TouchableOpacity onPress={() => handleFavorite(item.id, item.isFavorited)} style={styles.favBtn}>
                                        <Heart size={22} color={item.isFavorited ? '#FF4B4B' : theme.text.secondary}
                                            fill={item.isFavorited ? '#FF4B4B' : 'transparent'} />
                                    </TouchableOpacity>
                                </View>

                                {item.description ? (
                                    <Text style={[styles.courseDesc, { color: theme.text.secondary }]} numberOfLines={2}>{item.description}</Text>
                                ) : null}

                                <View style={styles.courseStats}>
                                    <View style={styles.statBadge}>
                                        <Star size={13} color="#FFD700" fill="#FFD700" />
                                        <Text style={styles.statText}>{item.rating.toFixed(1)} ({item.ratingCount})</Text>
                                    </View>
                                    <View style={styles.statBadge}>
...
const styles = StyleSheet.create({
    ...
    searchBox: { flexDirection: 'row', alignItems: 'center', borderRadius: 14, paddingHorizontal: 14, gap: 8 },
    searchInput: { flex: 1, height: 44, fontSize: 15 },

    sortRow: { flexDirection: 'row', paddingHorizontal: 16, paddingVertical: 8, gap: 8 },
    sortTab: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: '#333' },
