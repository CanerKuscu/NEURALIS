...
import { wallOfFameService, WallOfFameData, FameEntry } from '../src/services/WallOfFameService';
...
export default function WallOfFameScreen() {
    ...
    const getEntries = (): FameEntry[] => {
        if (!data) return [];
        switch (activeTab) {
            case 'streak': return data.topStreaks;
            case 'xp': return data.topXP;
            case 'lessons': return data.topLessons;
            case 'quests': return data.topQuests;
            default: return [];
        }
    };
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            {/* Golden Header */}
            <LinearGradient colors={['#2C1810', '#5C3D2E', '#8B6914']} style={[styles.header, { paddingTop: insets.top + 8 }]}>
                ...
                <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16 }}>
                    {entries.map((entry, i) => (
                        <Animated.View key={`${entry.userId}-${i}`} entering={FadeInDown.delay(i * 60)}>
                            <View style={[styles.rankRow, { backgroundColor: theme.background.secondary }, i < 3 && { borderLeftColor: RANK_COLORS[i], borderLeftWidth: 3 }]}>
                                <View style={[styles.rankBadge, { backgroundColor: i < 3 ? RANK_COLORS[i] + '30' : 'rgba(255,255,255,0.05)' }]}>
                                    {i === 0 ? <Crown size={16} color="#FFD700" /> :
                                        i === 1 ? <Medal size={16} color="#C0C0C0" /> :
                                            i === 2 ? <Medal size={16} color="#CD7F32" /> :
                                                <Text style={styles.rankNum}>{i + 1}</Text>}
                                </View>

                                <View style={styles.rankAvatar}>
                                    <Text style={styles.rankAvatarText}>{entry.username.charAt(0).toUpperCase()}</Text>
                                </View>

                                <View style={{ flex: 1 }}>
                                    <Text style={[styles.rankName, { color: theme.text.primary }]}>{entry.username}</Text>
                                    <Text style={[styles.rankValue, { color: currentTab.color }]}>{entry.label}</Text>
                                </View>

                                <Text style={[styles.rankScore, { color: currentTab.color }]}>{entry.value.toLocaleString()}</Text>
                            </View>
                        </Animated.View>
                    ))}
                    ...
                    {entries.length === 0 && (
                        <View style={styles.empty}>
                            ...
                            <Text style={[styles.emptyText, { color: theme.text.secondary }]}>Henüz veri yok</Text>
                        </View>
                    )}
...
function PodiumItem({ entry, rank, delay, isFirst }: { entry: FameEntry; rank: number; delay: number; isFirst?: boolean }) {
    const podiumH = isFirst ? 90 : rank === 2 ? 65 : 50;
    return (
        <Animated.View entering={FadeInUp.delay(delay)} style={[styles.podiumCol, { width: width / 3.5 }]}>
            ...
            <View style={[styles.podiumBar, { height: podiumH, backgroundColor: RANK_COLORS[rank - 1] + '60' }]}>
...
const styles = StyleSheet.create({
    ...
    backBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,215,0,0.1)', alignItems: 'center', justifyContent: 'center' },
    headerCenter: { alignItems: 'center' },
    headerTitle: { fontSize: 22, fontWeight: '800', color: '#FFD700' },
    headerSub: { color: 'rgba(255,215,0,0.7)', fontSize: 12, marginTop: 2 },

    podium: { flexDirection: 'row', justifyContent: 'center', alignItems: 'flex-end', marginTop: 12, paddingBottom: 8 },
    podiumCol: { alignItems: 'center', gap: 4 },
    podiumAvatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,215,0,0.2)', alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#FFD700' },
    ...
    crownBadge: { position: 'absolute', top: -12, backgroundColor: '#2C1810', borderRadius: 10, padding: 2 },
    ...
    podiumBar: { width: '80%', borderTopLeftRadius: 8, borderTopRightRadius: 8, alignItems: 'center', justifyContent: 'center' },
    ...
    tab: { flex: 1, paddingVertical: 8, borderRadius: 10, borderWidth: 1.5, borderColor: 'transparent', alignItems: 'center' },
    tabText: { fontSize: 11, fontWeight: '700', color: '#888' },

    rankRow: { flexDirection: 'row', alignItems: 'center', padding: 12, borderRadius: 14, marginBottom: 8, gap: 10 },
    ...
    rankAvatar: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.08)', alignItems: 'center', justifyContent: 'center' },
