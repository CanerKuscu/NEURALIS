...
import {
    ...
    Crown, Send, StopCircle, Play, MessageCircle,
                        ...
                        {VOICE_TOPICS.map((topic, idx) => (
                            <Animated.View key={topic.id} entering={FadeInDown.delay(idx * 50)}>
                                ...
                                >
                                    ...
                                    <Text style={[styles.topicTitle, { color: theme.text.primary }]}>{topic.titleTr}</Text>
                                    <Text style={[styles.topicDesc, { color: theme.text.secondary }]}>{topic.descriptionTr}</Text>
                                </TouchableOpacity>
                            </Animated.View>
                        ))}
                    </View>
                        ...
                        {(['beginner', 'intermediate', 'advanced'] as const).map((d) => (
                            ...
                            >
                                <Text style={[
                                    ...
                                    {d === 'beginner' ? i18n.t('voice.beginner') : d === 'intermediate' ? i18n.t('voice.intermediate') : i18n.t('voice.advanced')}
                                </Text>
                            </TouchableOpacity>
                        ))}
                    </View>
        ...
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary, paddingTop: insets.top }]}>
                ...
                >
                    ...
                    {isLoading && (
                        <View style={[styles.messageBubble, styles.aiBubble, { backgroundColor: theme.background.secondary }]}>
                ...
                {/* Input */}
                <View style={[styles.inputContainer, { backgroundColor: theme.background.secondary, paddingBottom: insets.bottom + 8 }]}>
                    <TextInput
                        style={[styles.textInput, { color: theme.text.primary, backgroundColor: theme.background.primary }]}
                    ...
                    <LinearGradient colors={['#6C5CE7', '#A18CD1']} style={styles.summaryGradient}>
                        ...
                        <View style={styles.summaryStats}>
                            <View style={styles.summaryStat}>
                                <Text style={styles.summaryStatValue}>{messages.filter(m => m.role === 'user').length}</Text>
                                <Text style={styles.summaryStatLabel}>Mesaj</Text>
                            </View>
                            <View style={styles.summaryStat}>
                                <Text style={styles.summaryStatValue}>
                                    {messages.filter(m => m.role === 'user').reduce((sum, m) => sum + m.text.split(' ').length, 0)}
                                </Text>
