...
import { ArrowLeft, Lock, Check, Star, ChevronRight, Play, BookOpen, Award, Sparkles } from 'lucide-react-native';
import Animated, { FadeInDown, FadeInRight } from 'react-native-reanimated';
import { useTheme } from '../src/context/ThemeContext';
import { storyModeService, STORY_WORLDS } from '../src/services/StoryModeService';
import type { StoryWorld, StoryChapter, StoryProgress, StoryChoice, StoryQuestion } from '../src/services/StoryModeService';
...
export default function StoryModeScreen() {
    ...
    const [selectedWorld, setSelectedWorld] = useState<(StoryWorld & { progress?: StoryProgress }) | null>(null);
    const [activeChapter, setActiveChapter] = useState<StoryChapter | null>(null);
    const [phase, setPhase] = useState<'worlds' | 'chapters' | 'narrative' | 'choice' | 'quiz' | 'result'>('worlds');
    ...
    const completeChapter = async (choice: StoryChoice | null) => {
        ...
        const result = await storyModeService.completeChapter(selectedWorld.id, activeChapter.id, choice?.id);
                                ...
                                <TouchableOpacity activeOpacity={0.9} onPress={() => selectWorld(world)}>
                                    <LinearGradient colors={[world.color, world.bgColor]} style={styles.worldCard}>
                                        ...
                                        <View style={styles.worldProgress}>
                                            ...
                                            <Text style={styles.worldProgressText}>{completedChapters}/{totalChapters}</Text>
                                        </View>
                                    </LinearGradient>
                                </TouchableOpacity>
                            </Animated.View>
        ...
        return (
            <View style={[styles.container, { paddingTop: insets.top, backgroundColor: theme.background.primary }]}>
                ...
                <View style={styles.header}>
                    ...
                    <Text style={[styles.headerTitle, { color: theme.text.primary }]}>{selectedWorld.emoji} {selectedWorld.titleTr}</Text>
                    <View style={{ width: 24 }} />
                </View>
                    ...
                    {selectedWorld.chapters.map((ch, i) => (
                        <Animated.View key={ch.id} entering={FadeInRight.delay(i * 100)}>
                            <TouchableOpacity
                                ...
                                style={[styles.chapterCard, { backgroundColor: theme.background.secondary, opacity: ch.isLocked ? 0.5 : 1 }]}
                            >
                                <View style={[styles.chapterNum, { backgroundColor: ch.isCompleted ? '#2ECC71' : ch.isLocked ? theme.border.light : selectedWorld.color }]}>
                                    {ch.isCompleted ? <Check size={18} color="#FFF" /> :
                                        ch.isLocked ? <Lock size={18} color={theme.text.secondary} /> :
                                            <Text style={styles.chapterNumText}>{ch.order}</Text>}
                                </View>
                                <View style={{ flex: 1 }}>
                                    <Text style={[styles.chapterTitle, { color: theme.text.primary }]}>{ch.titleTr}</Text>
                                    <Text style={[styles.chapterReward, { color: theme.text.secondary }]}>+{ch.xpReward} XP • +{ch.gemReward} 💎</Text>
                                </View>
                ...
                <ScrollView contentContainerStyle={{ padding: 24, paddingBottom: 100 }}>
                    ...
                    {choiceMade ? (
                        <Animated.View entering={FadeInDown.delay(200).springify()} style={styles.outcomeBox}>
                            ...
                            {choiceMade.xpBonus > 0 && <Text style={styles.outcomeXp}>+{choiceMade.xpBonus} XP Bonus!</Text>}
                        </Animated.View>
                    ) : (
        ...
        return (
            <View style={[styles.container, { paddingTop: insets.top, backgroundColor: theme.background.primary }]}>
                ...
                <View style={styles.header}>
                    ...
                    <Text style={[styles.progress, { color: theme.text.secondary }]}>{quizIndex + 1}/{activeChapter.questions.length}</Text>
                </View>

                <View style={{ padding: 24, gap: 16, flex: 1 }}>
                    <Text style={[styles.quizQuestion, { color: theme.text.primary }]}>{q.questionTr}</Text>
                    {q.options.map((opt, i) => (
                        <TouchableOpacity key={i} style={[styles.quizOption, { backgroundColor: theme.background.secondary }]} onPress={() => answerQuiz(i)}>
        ...
        return (
            <View style={[styles.container, { paddingTop: insets.top, backgroundColor: theme.background.primary }]}>
                ...
                <View style={styles.resultContainer}>
                    ...
                    <Text style={[styles.resultTitle, { color: theme.text.primary }]}>{i18n.t('story.chapter_completed')}</Text>
                    <View style={styles.rewardRow}>
                        <View style={styles.rewardItem}>
                            <Star size={24} color="#F1C40F" fill="#F1C40F" />
                            <Text style={[styles.rewardText, { color: theme.text.primary }]}>+{chapterResult?.xp || 0} XP</Text>
                        </View>
                        <View style={styles.rewardItem}>
                            <Sparkles size={24} color="#3498DB" />
                            <Text style={[styles.rewardText, { color: theme.text.primary }]}>+{chapterResult?.gems || 0} 💎</Text>
                        </View>
                    </View>
                    <TouchableOpacity style={styles.continueBtn} onPress={() => { setPhase('chapters'); setActiveChapter(null); }}>
...
const styles = StyleSheet.create({
    ...
    worldProgressBar: { flex: 1, height: 6, backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 3, overflow: 'hidden' },
    ...
    chapterCard: { flexDirection: 'row', alignItems: 'center', gap: 14, padding: 16, borderRadius: 16 },
    ...
    narrativeChapter: { color: '#F1C40F', fontSize: 14, fontWeight: '700', marginBottom: 8, letterSpacing: 1 },
    ...
    choiceBtn: { backgroundColor: 'rgba(255,255,255,0.1)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)', borderRadius: 16, padding: 18 },
    choiceBtnText: { color: '#FFF', fontSize: 15, fontWeight: '600', textAlign: 'center' },
    outcomeBox: { backgroundColor: 'rgba(46,204,113,0.15)', borderRadius: 16, padding: 20, marginTop: 20, gap: 8 },
    ...
    resultContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', gap: 20, padding: 40 },
    resultBadge: { width: 100, height: 100, borderRadius: 50, justifyContent: 'center', alignItems: 'center' },
    ...
    continueBtn: { backgroundColor: '#2ECC71', paddingHorizontal: 40, paddingVertical: 14, borderRadius: 16, marginTop: 8 },
