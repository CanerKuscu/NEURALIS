...
export default function LegalScreen() {
  ...
  const [activeTab, setActiveTab] = useState<Tab>(tab === 'terms' ? 'terms' : 'privacy');
  ...
  return (
    <>
      ...
      <Section title="4. Veri Güvenliği">
        ...
        (RLS) ile veritabanı erişim kontrolü{'\n'}• Düzenli güvenlik denetimleri
      </Section>

      <Section title="5. Çocuk Gizliliği (COPPA)">
        Neuralis, 13 yaş altı kullanıcılardan bilerek kişisel veri toplamaz. Eğer 13 yaş altı bir
        kullanıcının veri paylaştığını öğrenirsek, bu verileri derhal sileriz.
      </Section>

      <Section title="6. Kullanıcı Hakları">
        Aşağıdaki haklara sahipsiniz:{'\n\n'}• Verilerinize erişim talep etme{'\n'}• Verilerinizin
        düzeltilmesini isteme{'\n'}• Hesabınızı ve tüm verilerinizi silme{'\n'}• Veri
        taşınabilirliği (dışa aktarma){'\n\n'}
        Bu hakları kullanmak için uygulama içi ayarlardan veya destek ekibimizle iletişime geçerek
  ...
  return (
    <>
      ...
      <Section title="1. Kabul">
        Neuralis uygulamasını kullanarak bu Kullanım Koşullarını kabul etmiş olursunuz. Koşulları
        kabul etmiyorsanız uygulamayı kullanmayınız.
      </Section>

      <Section title="2. Hizmet Tanımı">
        Neuralis, yapay zeka destekli kişiselleştirilmiş bir öğrenme platformudur. Uygulama
        interaktif dersler, quiz'ler, ilerleme takibi ve sosyal öğrenme özellikleri sunar.
      </Section>

      <Section title="3. Hesap Sorumlulukları">
        • Hesap bilgilerinizin gizliliğinden siz sorumlusunuz{'\n'}• Hesabınızda gerçekleşen tüm
        aktivitelerden siz sorumlusunuz{'\n'}• Yetkisiz erişim durumunda derhal bizi
        bilgilendirmelisiniz{'\n'}• Her kullanıcı yalnızca bir hesap oluşturabilir
      </Section>

      <Section title="4. Premium Abonelik">
        • Premium özellikler aylık veya yıllık abonelikle sunulur{'\n'}• Abonelikler mevcut dönem
        sonunda otomatik yenilenir{'\n'}• İptal, yenileme tarihinden en az 24 saat önce yapılmalıdır
        {'\n'}• İadeler App Store / Google Play politikalarına tabidir{'\n'}• Ücretsiz deneme süresi
        içinde iptal edilirse ücret alınmaz
      </Section>

      <Section title="5. Kabul Edilebilir Kullanım">
        Aşağıdaki davranışlar yasaktır:{'\n\n'}• Uygulamayı yasadışı amaçlarla kullanmak{'\n'}•
        Diğer kullanıcılara zarar vermek, taciz veya spam{'\n'}• Sistem güvenliğini ihlal etmeye
        çalışmak{'\n'}• Uygulamayı tersine mühendislik yapmak{'\n'}• Sahte hesaplar oluşturmak veya
        bot kullanmak
      </Section>

      <Section title="6. Fikri Mülkiyet">
        Neuralis uygulamasının tüm içeriği, tasarımı ve kodu telif hakkı ile korunmaktadır. İçerik
        izinsiz kopyalanamaz, dağıtılamaz veya ticari amaçla kullanılamaz.
      </Section>

      <Section title="7. Sorumluluk Sınırlaması">
        Neuralis "olduğu gibi" sunulmaktadır. Hizmet kesintileri, veri kayıpları veya doğruluk
        hataları nedeniyle oluşabilecek doğrudan veya dolaylı zararlardan sorumlu değiliz. Uygulama
        eğitim amaçlıdır ve profesyonel tavsiye yerine geçmez.
      </Section>

      <Section title="8. Değişiklikler">
        Bu koşulları önceden bildirimde bulunarak değiştirme hakkını saklı tutarız. Değişiklikler
