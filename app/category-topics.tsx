...
export default function CategoryTopicsScreen() {
  ...
  const loadUserLevels = async () => {
    try {
      ...
      const subIds = categoryData.subcategories.map((sc) => buildSubjectKey(category!, sc.id));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.topicCount}>{categoryData.subcategories.length} konu</Text>
      </LinearGradient>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.listContent}>
        {categoryData.subcategories.map((sub, index) => {
          ...
          return (
            <Animated.View key={sub.id} entering={FadeInDown.delay(index * 80).springify()}>
              <TouchableOpacity
              ...
              >
                ...
                <View style={styles.topicText}>
                  <Text style={[styles.topicName, { color: theme.text.primary }]}>{sub.name}</Text>
