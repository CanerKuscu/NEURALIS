...
import { StyleSheet, View, Text, TouchableOpacity, ScrollView, RefreshControl, Dimensions } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { useFocusEffect } from '@react-navigation/native';
import { Settings, Edit2, Flame, Zap, Trophy, UserPlus, Share2, Users, BookOpen, X, Target, Camera, Star } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { supabase } from '../../src/config/supabase';
import { useTheme } from '../../src/context/ThemeContext';
import AvatarV2View from '../../src/components/avatar/AvatarV2View';
import { AvatarV2Config, DEFAULT_AVATAR_V2 } from '../../src/types/avatar-v2';
...
const FriendSuggestionCard = React.memo(({ item, index, theme }: { item: FriendSuggestion; index: number; theme: ReturnType<typeof useTheme>['theme'] }) => (
    <Animated.View
        entering={FadeInRight.delay(index * 100)}
        style={[styles.suggestionCard, { backgroundColor: theme.background.secondary, borderColor: theme.border.light }]}
    >
        ...
        <Text style={[styles.suggestionInfo, { color: theme.text.secondary }]}>
            {item.followsYou ? i18n.t('profile.follows_you') : i18n.t('profile.mutual_friends', { count: item.mutualFriends })}
        </Text>
...
export default function ProfileScreen() {
    ...
    const [showcaseBadges, setShowcaseBadges] = useState<{ id: string; icon: string; title: string; color: string }[]>([]);
    ...
    const loadProfile = async () => {
        ...
        if (session) {
            ...
            const { data: friends } = await supabase
                .from('friendships')
                .select('friend_id, profiles!friendships_friend_id_fkey(id, display_name, username, avatar_config)')
                .eq('user_id', session.user.id)
                .eq('status', 'accepted')
                .limit(5);

            if (friends && friends.length > 0) {
                setFriendSuggestions(friends.map((f: any) => ({
                    id: f.friend_id,
            ...
            if (userCourses && userCourses.length > 0) {
                setCourses(userCourses.map((c: any, i: number) => ({
                    id: String(i),
                    name: c.category,
    ...
    const loadShowcaseBadges = async () => {
        try {
            ...
            if (stored) {
                ...
                setShowcaseBadges(ids.map(id => ({ id, ...(badgeMap[id] || { icon: '🏅', title: id, color: '#888' }) })));
            }
        } catch (e) { }
    };
    ...
    // Stats data
    const stats = [
        { label: 'Courses', value: courses.length, extra: courses.length > 2 ? `+${courses.length - 2}` : null },
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            <ScrollView
                ...
                refreshControl={<RefreshControl refreshing={loading} onRefresh={loadProfile} tintColor={theme.primary} />}
            >
                {/* Large Banner with Avatar - Duolingo Style */}
                <View style={styles.bannerSection}>
                    <LinearGradient
                        colors={[avatarConfig.bgColor || '#9B59B6', adjustColor(avatarConfig.bgColor || '#9B59B6', -30)]}
                        ...
                        <View style={styles.bannerAvatarContainer}>
                            <Animated.View entering={FadeIn.delay(200).springify()}>
                                ...
                                >
                                    ...
                                    <View style={[styles.editBadge, { backgroundColor: theme.primary, borderColor: theme.background.primary }]}>
                ...
                <View style={styles.statsRow}>
                    ...
                    <View style={styles.statItem}>
                        <View style={styles.courseFlags}>
                            {courses.slice(0, 2).map((course, i) => (
                                <Text key={course.id} style={styles.courseFlag}>{course.flag}</Text>
                            ))}
                            {courses.length > 2 && (
                                <View style={[styles.extraBadge, { backgroundColor: theme.background.tertiary }]}>
                                    <Text style={[styles.extraText, { color: theme.text.secondary }]}>+{courses.length - 2}</Text>
                                </View>
                            )}
                        </View>
                        <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('profile.courses')}</Text>
                    </View>
                    ...
                    <View style={styles.statItem}>
                        ...
                        <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('profile.following')}</Text>
                    </View>
                    <View style={styles.statDivider} />
                    <View style={styles.statItem}>
                        <Text style={[styles.statValue, { color: theme.text.primary }]}>
                        ...
                        <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('profile.followers')}</Text>
                    </View>
                </View>
                ...
                <View style={styles.actionButtons}>
                    ...
                    >
                        ...
                        <Text style={[styles.addFriendsText, { color: theme.primary }]}>{i18n.t('profile.add_friends')}</Text>
                    </TouchableOpacity>
                ...
                {friendSuggestions.length > 0 && (
                    <View style={styles.section}>
                        <View style={styles.sectionHeader}>
                            <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>{i18n.t('profile.friends')}</Text>
                            <TouchableOpacity onPress={() => router.push('/(tabs)/social')}>
                                <Text style={[styles.viewAll, { color: theme.primary }]}>{i18n.t('profile.view_all')}</Text>
                            </TouchableOpacity>
                        </View>
                        <ScrollView
                            horizontal
                ...
                {/* Overview Section */}
                <View style={styles.section}>
                    <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>{i18n.t('profile.statistics')}</Text>
                    <View style={styles.overviewGrid}>
                        <Animated.View entering={FadeInDown.delay(100)} style={[styles.overviewItem, { backgroundColor: theme.background.secondary }]}>
                        ...
                        <Animated.View entering={FadeInDown.delay(200)} style={[styles.overviewItem, { backgroundColor: theme.background.secondary }]}>
                        ...
                        <Animated.View entering={FadeInDown.delay(300)} style={[styles.overviewItem, { backgroundColor: theme.background.secondary }]}>
                        ...
                        <Animated.View entering={FadeInDown.delay(400)} style={[styles.overviewItem, { backgroundColor: theme.background.secondary }]}>
                    ...
                    <View style={styles.section}>
                        ...
                        <View style={styles.showcaseRow}>
                            {showcaseBadges.map((badge, idx) => (
                                <Animated.View key={badge.id} entering={ZoomIn.delay(idx * 150)} style={[styles.showcaseBadge, { backgroundColor: `${badge.color}20`, borderColor: badge.color }]}>
                                    <Text style={{ fontSize: 28 }}>{badge.icon}</Text>
                                    <Text style={[styles.showcaseBadgeText, { color: badge.color }]}>{badge.title}</Text>
                                </Animated.View>
                            ))}
                        </View>
                    </View>
                ...
                <View style={styles.section}>
                    <View style={styles.sectionHeader}>
                        <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>{i18n.t('profile.achievements')}</Text>
