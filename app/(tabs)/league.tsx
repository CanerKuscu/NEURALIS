...
import type { LeagueRank } from '../../src/components/league/LeagueBadge';
import { LeagueBadge, RANK_NAMES } from '../../src/components/league/LeagueBadge';
...
export default function LeagueScreen() {
  ...
  const [history, setHistory] = useState<
    { week: string; rank: number; league: LeagueRank; xp: number; promoted: boolean }[]
  >([]);
  ...
  const loadLeagueData = async () => {
    try {
      ...
      const finalLeaders: LeaderboardEntry[] = (leaders && leaders.length > 0 ? leaders : []).map(
        (l, i) => ({ ...l, rank: i + 1 }),
      );
      ...
      setHistory([
        ...
        {
          week: i18n.t('league.weeks_ago', { count: 2 }),
          rank: 7,
          league: 'bronze' as LeagueRank,
          xp: 310,
          promoted: false,
        },
        {
          week: i18n.t('league.weeks_ago', { count: 3 }),
          rank: 1,
          league: 'bronze' as LeagueRank,
          xp: 890,
          promoted: true,
        ...
        {
          week: i18n.t('league.weeks_ago', { count: 4 }),
          rank: 4,
          league: 'bronze' as LeagueRank,
          xp: 520,
          promoted: false,
      ...
      <Animated.View entering={FadeInDown.delay(index * 50).springify()}>
        <View
          style={[
            ...
            isMe && styles.myCard,
            getRankStyle(item.rank || index + 1),
          ]}
        >
          <View style={styles.rankContainer}>{getRankIcon(item.rank || index + 1)}</View>

          <View style={styles.avatarContainer}>
            {/* Fallback Avatar */}
            <View
              style={[styles.avatarPlaceholder, { backgroundColor: theme.background.tertiary }]}
            >
          ...
          <View style={styles.infoContainer}>
            <Text
              style={[
                ...
                isMe && { color: theme.primary },
              ]}
            >
              {item.display_name || item.username || i18n.t('league.anonymous')}
            </Text>
            <Text style={[styles.xpText, { color: theme.text.secondary }]}>
              {item.league_points} XP
            </Text>
          </View>
  ...
  return (
    ...
    >
      ...
      <LinearGradient colors={getLeagueGradient(userLeague)} style={styles.headerGradient}>
      ...
      <View style={[styles.tabContainer, { backgroundColor: theme.background.secondary }]}>
        ...
        >
          ...
          <Text
            style={[
              styles.tabText,
              { color: activeTab === 'leaderboard' ? '#FFF' : theme.text.secondary },
            ]}
          >
            {i18n.t('league.ranking')}
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
        ...
        >
          ...
          <Text
            style={[
              styles.tabText,
              { color: activeTab === 'history' ? '#FFF' : theme.text.secondary },
            ]}
          >
            {i18n.t('league.history')}
          </Text>
        </TouchableOpacity>
      </View>
            ...
            <Animated.View key={idx} entering={FadeInDown.delay(idx * 80)}>
              <View style={[styles.historyCard, { backgroundColor: theme.background.secondary }]}>
                <View style={styles.historyLeft}>
                  ...
                  <View>
                    <Text style={[styles.historyWeek, { color: theme.text.primary }]}>
                      {h.week}
                    </Text>
                    <Text style={[styles.historyLeague, { color: theme.text.secondary }]}>
                      {RANK_NAMES[h.league]}
                    </Text>
                  </View>
                </View>
                  ...
                  <View style={styles.historyRank}>
                    <Text
                      style={[
                        ...
                        { color: h.rank <= 3 ? '#F1C40F' : theme.text.primary },
                      ]}
                    >
                      #{h.rank}
                    </Text>
