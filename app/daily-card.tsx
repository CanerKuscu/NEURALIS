...
import { StyleSheet, View, Text, TouchableOpacity, StatusBar, Dimensions, Share } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { LinearGradient } from 'expo-linear-gradient';
import { router } from 'expo-router';
import { ArrowLeft, Heart, Share2, ChevronLeft, ChevronRight, BookOpen, Calendar } from 'lucide-react-native';
    ...
    return (
        <View style={[styles.container, { paddingTop: insets.top, backgroundColor: theme.background.primary }]}>
            ...
            <View style={styles.header}>
                ...
                <Text style={[styles.headerTitle, { color: theme.text.primary }]}>{i18n.t('daily.title')}</Text>
                <View style={{ width: 24 }} />
            </View>
            ...
            <Animated.View entering={FadeIn.delay(300)} style={styles.actions}>
                <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.background.secondary }]} onPress={toggleFav}>
                    <Heart size={24} color={isFavorite ? '#E74C3C' : theme.text.secondary} fill={isFavorite ? '#E74C3C' : 'transparent'} />
                    <Text style={[styles.actionText, { color: theme.text.secondary }]}>Favori</Text>
                </TouchableOpacity>

                <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.background.secondary }]} onPress={shareCard}>
                    <Share2 size={24} color={theme.text.secondary} />
                    <Text style={[styles.actionText, { color: theme.text.secondary }]}>Paylaş</Text>
                </TouchableOpacity>

                <TouchableOpacity style={[styles.actionBtn, { backgroundColor: theme.background.secondary }]} onPress={() => router.push('/spaced-repetition')}>
                ...
                <View style={styles.historySection}>
                    ...
                    <View style={styles.historyRow}>
                        {history.slice(1, 5).map((h, i) => {
                            ...
                            return (
                                <View key={h.id} style={[styles.historyCard, { backgroundColor: theme.background.secondary }]}>
                                    <Text style={styles.historyEmoji}>{hCat?.emoji}</Text>
                                    <Text style={[styles.historyCardTitle, { color: theme.text.primary }]} numberOfLines={1}>{h.title}</Text>
                                </View>
                            );
                        })}
                    </View>
...
const styles = StyleSheet.create({
    ...
    categoryBadge: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: 'rgba(255,255,255,0.15)', alignSelf: 'flex-start', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 20 },
    ...
    actions: { flexDirection: 'row', justifyContent: 'center', gap: 16, paddingVertical: 20, paddingHorizontal: 20 },
    ...
    historyRow: { gap: 8 },
    historyCard: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: 14 },
