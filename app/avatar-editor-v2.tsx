...
import {
    X, Check, Undo2, Redo2, RotateCcw, Shuffle,
...
import {
    AvatarV2Config,
    DEFAULT_AVATAR_V2,
    AVATAR_V2_CATEGORIES,
    getOptionLabelKey,
} from '../src/types/avatar-v2';
...
export default function AvatarEditorV2() {
    ...
    const updateConfig = useCallback((key: keyof AvatarV2Config, value: string) => {
        setConfig(prev => {
                    ...
                    {AVATAR_V2_CATEGORIES.map((cat, idx) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <Text
                                    style={[styles.catLbl, active && styles.catLblOn]}
                                    numberOfLines={1}
                                >
                                    {i18n.t(cat.labelKey)}
                                </Text>
...
const OptionCard = React.memo(({
    opt, isSelected, config, configKey, onSelect, index,
}: {
    opt: string;
    isSelected: boolean;
    ...
    const previewConfig = useMemo(() => ({ ...config, [configKey]: opt }), [config, configKey, opt]);
...
function OptionGrid({ options, selected, onSelect, configKey, config }: {
    options: string[];
    selected: string;
    onSelect: (v: string) => void;
    configKey: keyof AvatarV2Config;
...
const styles = StyleSheet.create({
    ...
    oGrid: {
        flexDirection: 'row', flexWrap: 'wrap',
        gap: OPTION_GAP, justifyContent: 'flex-start',
    },
    oCard: {
    ...
    oPrev: {
        width: MINI_AVATAR, height: MINI_AVATAR,
    ...
    saveBtn: {
        ...
        borderBottomWidth: 4, borderBottomColor: C.greenDark,
        ...Platform.select({
            ios: {
                shadowColor: C.green,
