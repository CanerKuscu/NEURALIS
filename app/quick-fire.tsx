...
import { StyleSheet, View, Text, TouchableOpacity, Dimensions, ActivityIndicator, Vibration } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { ArrowLeft, Zap, Timer, Trophy, Target, Flame, RotateCcw } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import Animated, { FadeInDown, FadeIn, ZoomIn, useSharedValue, useAnimatedStyle, withTiming, withSequence, withSpring } from 'react-native-reanimated';
import { useTheme } from '../src/context/ThemeContext';
import { supabase } from '../src/config/supabase';
import { quickFireService, QuickFireRound, QuickFireQuestion } from '../src/services/QuickFireService';
...
export default function QuickFireScreen() {
    ...
    useEffect(() => { loadRound(); return () => { if (timerRef.current) clearInterval(timerRef.current); }; }, []);
    ...
    if (gameState === 'loading') {
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                ...
                <Text style={[styles.loadingText, { color: theme.text.secondary }]}>⚡ Sorular hazırlanıyor...</Text>
            </View>
        );
    }
    ...
    if (gameState === 'countdown') {
        return (
            <View style={[styles.container, styles.center, { backgroundColor: theme.background.primary }]}>
                <Animated.Text entering={ZoomIn} style={styles.countdownText}>{countdown}</Animated.Text>
    ...
    if (gameState === 'result') {
        ...
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                <LinearGradient colors={['#FF6B00', '#FF8C00']} style={[styles.resultHeader, { paddingTop: insets.top + 20 }]}>
                ...
                <View style={styles.resultBody}>
                    <Animated.View entering={FadeInDown.delay(200)} style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                        <Target size={24} color="#2ECC71" />
                        <Text style={[styles.statValue, { color: theme.text.primary }]}>{score}</Text>
                        <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('quickfire.total_score')}</Text>
                    </Animated.View>
                    <Animated.View entering={FadeInDown.delay(300)} style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                        ...
                        <Text style={[styles.statValue, { color: theme.text.primary }]}>{correct}/{round?.questions.length}</Text>
                        <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('quickfire.correct')}</Text>
                    </Animated.View>
                    <Animated.View entering={FadeInDown.delay(400)} style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                        ...
                        <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('quickfire.accuracy')}</Text>
                    </Animated.View>
                </View>
                <View style={styles.resultActions}>
                    <TouchableOpacity style={styles.retryBtn} onPress={() => { setGameState('loading'); setScore(0); setCorrect(0); setCurrentQ(0); setTimeLeft(TOTAL_TIME); progressWidth.value = 1; loadRound(); }}>
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            ...
            <View style={[styles.topBar, { paddingTop: insets.top + 8 }]}>
                <TouchableOpacity onPress={() => { if (timerRef.current) clearInterval(timerRef.current); router.back(); }} style={styles.closeBtn}>
                    <ArrowLeft size={22} color={theme.text.primary} />
                </TouchableOpacity>
                <View style={styles.timerContainer}>
                    <Timer size={18} color={timeLeft <= 10 ? '#FF4B4B' : '#FF6B00'} />
                    <Text style={[styles.timerText, timeLeft <= 10 && { color: '#FF4B4B' }]}>{timeLeft}s</Text>
                </View>
            ...
            {/* Question counter */}
            <Text style={[styles.questionCounter, { color: theme.text.secondary }]}>{currentQ + 1} / {round?.questions.length}</Text>

            {/* Question */}
            <Animated.View style={[styles.questionCard, shakeStyle, { backgroundColor: theme.background.secondary }]}>
            ...
            <View style={styles.optionsContainer}>
                {question.options.map((opt, i) => {
                    ...
                    let border = '#333';
                    if (showFeedback) {
                        if (i === question.correctAnswer) { bg = '#2ECC7133'; border = '#2ECC71'; }
                        else if (i === selectedAnswer && i !== question.correctAnswer) { bg = '#FF4B4B33'; border = '#FF4B4B'; }
                    }
                    return (
...
const styles = StyleSheet.create({
    ...
    timerContainer: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(255,107,0,0.15)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 16 },
    ...
    questionCard: { margin: 16, borderRadius: 20, padding: 20, minHeight: 120, justifyContent: 'center' },
    ...
    retryBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#FF6B00', borderRadius: 14, padding: 16 },
