...
import {
    ...
    ActivityIndicator, ScrollView, Image, Dimensions, Modal, TextInput, Vibration
} from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useLocalSearchParams, router } from 'expo-router';
import TTSButton from '../src/components/TTSButton';
import { ArrowLeft, CheckCircle, XCircle, Zap, ChevronRight, Heart, BookOpen, X, Flame, Star, Trophy, Award, Target, Lightbulb, Volume2 } from 'lucide-react-native';
import Animated, {
    useSharedValue,
    useAnimatedStyle,
    withTiming,
...
type LessonPhase = 'loading' | 'placement-test' | 'result' | 'lesson' | 'theory' | 'error' | 'streak-celebration' | 'daily-limit' | 'ad-reward';
...
export default function LessonScreen() {
    ...
    const bgAnimStyle = useAnimatedStyle(() => {
        const bgColor = bgColorAnim.value === 1
            ? COLORS.correctBg
            : bgColorAnim.value === -1
                ? COLORS.wrongBg
                : COLORS.bg;
    ...
    const checkUserLevel = async () => {
        try {
            ...
            const { data: profile } = await supabase.from('profiles').select('age, streak_count').eq('id', session.user.id).single();
            if (profile) {
...
Write all questions in ${langName}.`,
            ...
            setLessonTitle(lessonData.title || `${i18n.t('lesson.placement_test', { defaultValue: 'Placement Test' })}: ${subject}`);
            ...
            setTheoryContent(i18n.t('lesson.ai_busy_fallback', { defaultValue: 'AI is busy right now, but here is a practice set we prepared for you!' }));
            setPhase('theory');
        }
    };
    ...
    const generateLesson = async (level: string) => {
        ...
        try {
            ...
            if (cacheResult.found && cacheResult.lesson) {
                ...
                setQuestions(normalized);
                setLessonTitle(cached.title || `${subject} ${i18n.t('lesson.lesson_title', { defaultValue: 'Lesson' })}`);
                setCurrentIndex(0);
                setScore(0);
                ...
                const personalIntro = lessonCacheService.generatePersonalizedIntro(
                    null, dailyStreak, subject, level
                );
                const theory = cached.theory
                    ? `${personalIntro}\n\n${cached.theory}`
                    : personalIntro;
...
Focus on real-world applications and examples.`,
            ...
            setQuestions(normalized);
            setLessonTitle(lessonData.title || `${subject} ${i18n.t('lesson.lesson_title', { defaultValue: 'Lesson' })}`);
            setCurrentIndex(0);
            setScore(0);
            setSelectedAnswer(null);
            setIsAnswered(false);
            ...
            lessonCacheService.cacheLesson({
                category: subject,
                difficulty: level,
    ...
    const handleEli5 = async () => {
        ...
        try {
            ...
            setEli5Text(data?.explanation || data?.text || data?.content || 'Açıklama üretildi ama bir sorun oluştu.');
        } catch (e) {
    ...
    const handleAnswer = async (index: number) => {
        ...
        if (isCorrect) {
            ...
            if (currentQuestion) {
                setWrongQuestionQueue(prev => [...prev, { ...currentQuestion, id: `${currentQuestion.id}_retry_${Date.now()}` }]);
            }
        }
    };
    ...
    const handleOrderSubmit = () => {
        ...
        if (isCorrect) {
            bgColorAnim.value = withSequence(withTiming(1, { duration: 200 }), withDelay(800, withTiming(0, { duration: 300 })));
            setScore(prev => prev + 1);
            setLessonStreak(prev => prev + 1);
            triggerCorrectEffect();
        } else {
            bgColorAnim.value = withSequence(withTiming(-1, { duration: 200 }), withDelay(800, withTiming(0, { duration: 300 })));
            shakeAnim.value = withSequence(withTiming(-10, { duration: 50 }), withTiming(10, { duration: 50 }), withTiming(-10, { duration: 50 }), withTiming(10, { duration: 50 }), withTiming(0, { duration: 50 }));
            setLessonStreak(0);
            triggerWrongEffect();
            // Add wrong question to re-ask queue
            if (currentQuestion) {
    ...
    const handleComplete = async () => {
        ...
        if (userId) {
            ...
            if (currentCacheId) {
                lessonCacheService.recordInteraction(
                    userId,
                    currentCacheId,
        ...
        <Animated.View style={[styles.container, { paddingTop: insets.top }, bgAnimStyle]}>
            ...
            {phase === 'daily-limit' && (
                <View style={styles.errorContainer}>
                    ...
                    <Text style={styles.dailyLimitInfo}>
                        {i18n.t('lesson_limit.lessons_completed', { completed: dailyLessonData?.lessonsCompletedToday || 0, total: dailyLessonData?.totalLessonsAvailable || 2 })}
                    </Text>
                    ...
                    <View style={styles.adSection}>
                        ...
                        <View style={styles.adProgressRow}>
                            {[0, 1, 2].map(i => {
                                const watchedInCurrent = dailyLessonData
                                    ? dailyLessonData.adsWatchedToday % 3
                                    : 0;
                                ...
                                return (
                                    <View
                                        key={i}
                                        style={[
                                            styles.adProgressDot,
                                            filled && styles.adProgressDotFilled
                                        ]}
                                    />
                                );
                            })}
                    ...
                    <View style={styles.premiumUpsell}>
                        <Text style={styles.premiumUpsellTitle}>{i18n.t('lesson_limit.unlimited_premium')}</Text>
            ...
            {phase === 'theory' && theoryContent && (
                <ScrollView contentContainerStyle={styles.theoryContainer}>
                    ...
                    <Animated.View entering={SlideInUp.delay(200)} style={styles.theoryContent}>
                        <Text style={styles.theoryText}>{theoryContent.replace(/## /g, '').replace(/\*\*/g, '')}</Text>
                    </Animated.View>
                    <Animated.View entering={FadeIn.delay(500)}>
                        ...
                        >
                            ...
                            >
                                <Text style={styles.startBtnText}>{i18n.t('lesson.start_quiz') || 'Teste Başla'}</Text>
            ...
            {(phase === 'placement-test' || phase === 'lesson') && currentQuestion && (
                ...
                >
                    ...
                    {showCorrectEffect && (
                        <Animated.View entering={ZoomIn.duration(300)} exiting={FadeOut.duration(400)} style={styles.correctEffectOverlay}>
                    ...
                    {showWrongEffect && (
                        <Animated.View entering={ZoomIn.duration(200)} exiting={FadeOut.duration(300)} style={styles.wrongEffectOverlay}>
                    ...
                    <Animated.View entering={FadeInDown.delay(50)} style={styles.questionTypeBadge}>
                        <Text style={styles.questionTypeBadgeText}>
                            {currentQuestion.type === 'multiple_choice' ? '📝 Çoktan Seçmeli' :
                                currentQuestion.type === 'true_false' ? i18n.t('true_false') :
                                    currentQuestion.type === 'fill_blank' ? '✏️ Boşluk Doldur' :
                                        currentQuestion.type === 'ordering' ? '🔢 Sırala' :
                                            currentQuestion.type === 'matching' ? '🔗 Eşleştir' : '📝 Soru'}
                        </Text>
                    </Animated.View>
                    ...
                    <Animated.View entering={FadeIn} style={styles.questionCard}>
                        ...
                        <Text style={styles.questionLabel}>
                            {phase === 'placement-test' ? i18n.t('level_test') : i18n.t('question_n', { current: currentIndex + 1, total: questions.length })}
                        </Text>
                        ...
                        <Animated.View entering={FadeInDown} style={styles.fillBlankContainer}>
                            <TextInput
                                style={[
                                    ...
                                    isAnswered && fillBlankInput.toLowerCase().trim() === (currentQuestion.correctText?.toLowerCase().trim() || '')
                                        ? styles.fillBlankCorrect
                                        : isAnswered ? styles.fillBlankWrong : null
                                ]}
                    ...
                    {currentQuestion.type === 'ordering' && (
                        <View style={styles.orderingContainer}>
                            ...
                            <View style={styles.orderSelectedArea}>
                                {selectedOrder.length > 0 ? selectedOrder.map((item, idx) => (
                                    ...
                                    return (
                                        <Animated.View key={`avail_${idx}`} entering={FadeInDown.delay(idx * 60)}>
                                            <TouchableOpacity
                                                style={[
                                                    styles.orderChip,
                                                    alreadySelected && styles.orderChipDisabled
                                                ]}
                                                onPress={() => !isAnswered && handleOrderSelect(item)}
                                                disabled={isAnswered || alreadySelected}
                                            >
                                                <Text style={[styles.orderChipText, alreadySelected && { opacity: 0.3 }]}>{item}</Text>
                                            </TouchableOpacity>
                                        </Animated.View>
                                    );
                                })}
                            ...
                            {isAnswered && (
                                <Animated.View entering={FadeIn} style={styles.orderCorrectAnswer}>
                                    ...
                                    {(currentQuestion.orderItems || []).map((item, idx) => (
                                        <Text key={idx} style={styles.orderCorrectItem}>{idx + 1}. {item}</Text>
                                    ))}
                                </Animated.View>
                            )}
                        </View>
                    ...
                    {currentQuestion.type === 'matching' && (
                        <View style={styles.matchingContainer}>
                            ...
                            {(currentQuestion.matchPairs || []).map((pair, idx) => (
                                <Animated.View key={idx} entering={FadeInDown.delay(idx * 80)} style={styles.matchRow}>
                                    <TouchableOpacity
                                        style={[
                                            styles.matchLeft,
                                            matchLeftSelected === pair.left && styles.matchLeftActive,
                                            isAnswered && matchSelections[pair.left] === pair.right && styles.matchCorrectBorder,
                                            isAnswered && matchSelections[pair.left] && matchSelections[pair.left] !== pair.right && styles.matchWrongBorder,
                                        ]}
                                    ...
                                    {(currentQuestion.matchPairs || []).map((pair, idx) => {
                                        ...
                                        return (
                                            <TouchableOpacity
                                                ...
                                                onPress={() => !alreadyUsed && handleMatchPair(matchLeftSelected!, pair.right)}
                            ...
                            {!isAnswered && Object.keys(matchSelections).length === (currentQuestion.matchPairs?.length || 0) && (
                    ...
                    {isAnswered && (
                        <Animated.View
                            style={[
                                ...
                                (currentQuestion.type === 'multiple_choice' || currentQuestion.type === 'true_false' || !currentQuestion.type)
                                    ...
                                    : (selectedAnswer === 1 ? styles.feedbackCorrect : styles.feedbackWrong)
                            ]}
                        ...
                        >
                            ...
                            {isAnswered && ((currentQuestion.type === 'multiple_choice' || currentQuestion.type === 'true_false' || !currentQuestion.type)
                                ? selectedAnswer !== currentQuestion.correctAnswer
                                : selectedAnswer !== 1) && (
                                    <View style={styles.eli5Container}>
                                        {eli5Text ? (
                                            ...
                                            <TouchableOpacity style={styles.eli5Btn} onPress={handleEli5} disabled={eli5Loading}>
                                                {eli5Loading ? (
                            ...
                            >
                                <Text style={styles.nextBtnText}>
                                    {currentIndex < questions.length - 1 ? i18n.t('common.continue') : i18n.t('common.finish')}
                                </Text>
                                <ChevronRight size={24} color="#FFF" />
                            </TouchableOpacity>
                        </Animated.View>
                ...
                <View style={styles.resultContainer}>
                    ...
                    >
                        <Animated.View entering={ZoomIn} style={styles.resultCard}>
                            ...
                            <Text style={styles.resultTitle}>{i18n.t('lesson.complete') || 'Ders Tamamlandı!'}</Text>
                            ...
                            <View style={styles.statsRow}>
                                ...
                                <View style={styles.statItem}>
                                    ...
                                    <Text style={styles.statValue}>{questions.length > 0 ? Math.round((score / questions.length) * 100) : 0}%</Text>
...
const styles = StyleSheet.create({
    ...
    fillBlankCorrectLabel: { fontSize: 12, color: COLORS.textSecondary, fontWeight: '600', marginBottom: 4 },
    ...
    orderCorrectLabel: { fontSize: 12, color: COLORS.textSecondary, fontWeight: '600', marginBottom: 8 },
    orderCorrectItem: { fontSize: 15, color: COLORS.correctGreen, fontWeight: '600', marginBottom: 4 },
