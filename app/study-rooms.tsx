...
import { StyleSheet, View, Text, TouchableOpacity, FlatList, TextInput, ActivityIndicator, KeyboardAvoidingView, Platform } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { ArrowLeft, Users, Plus, MessageCircle, Send, Play, Zap, BookOpen, Crown } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import Animated, { FadeInDown, FadeIn, ZoomIn } from 'react-native-reanimated';
import { useTheme } from '../src/context/ThemeContext';
import { supabase } from '../src/config/supabase';
import { studyRoomService, StudyRoom, RoomParticipant, RoomMessage } from '../src/services/StudyRoomService';
...
export default function StudyRoomsScreen() {
    ...
    useEffect(() => {
        loadData();
        return () => { if (currentRoom) studyRoomService.leaveRoom(currentRoom.id); };
    }, []);
    ...
    const handleCreateRoom = async () => {
        if (!newTopic.trim()) { modal.error(i18n.t('common.error'), i18n.t('study_room.enter_topic')); return; }
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
        const room = await studyRoomService.createRoom(userId, displayName, newCategory, newTopic.trim());
    ...
    const joinRoomHandler = (room: StudyRoom) => {
        ...
        studyRoomService.joinRoom(room.id, userId, displayName, {
            onParticipantJoin: (p) => setParticipants(prev => [...prev.filter(x => x.userId !== p.userId), p]),
        ...
        // System message
        setMessages([{ userId: 'system', displayName: 'System', type: 'system', text: `${displayName} odaya katıldı`, timestamp: new Date().toISOString() }]);
    };

    const handleSend = () => {
        if (!msgInput.trim() || !currentRoom) return;
        const msg: RoomMessage = { userId, displayName, type: 'chat', text: msgInput.trim(), timestamp: new Date().toISOString() };
    ...
    const handleStartGame = () => {
        ...
        router.push(`/lesson?category=${currentRoom.category}&topic=${currentRoom.topic}&roomId=${currentRoom.id}`);
    };
    ...
    if (screenState === 'lobby') {
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                <LinearGradient colors={['#1ABC9C', '#16A085']} style={[styles.header, { paddingTop: insets.top + 8 }]}>
                ...
                {loading ? (
                    ...
                    <View style={styles.emptyContainer}>
                        ...
                        <Text style={[styles.emptyText, { color: theme.text.secondary }]}>Henüz açık oda yok</Text>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => (
                            <Animated.View entering={FadeInDown.delay(index * 80)}>
                                <TouchableOpacity style={[styles.roomCard, { backgroundColor: theme.background.secondary }]}
                                    onPress={() => joinRoomHandler(item)}>
                                    <View style={styles.roomTop}>
                                        <Text style={[styles.roomTopic, { color: theme.text.primary }]}>{item.topic}</Text>
                                        <View style={styles.roomPlayerBadge}>
                                            <Users size={14} color="#1ABC9C" />
                                            <Text style={styles.roomPlayerCount}>{item.participants.length}/{item.maxPlayers}</Text>
                                        </View>
                                    </View>
                    ...
                    <View style={styles.categoryGrid}>
                        {CATEGORIES.map(cat => (
                            <TouchableOpacity key={cat.id}
                                ...
                                <Text style={[styles.catChipText, { color: newCategory === cat.id ? cat.color : theme.text.secondary }]}>
                                    {cat.label}
                                </Text>
                            </TouchableOpacity>
                        ))}
    ...
    // ─── ROOM ───
    return (
        <KeyboardAvoidingView style={[styles.container, { backgroundColor: theme.background.primary }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            <View style={[styles.simpleHeader, { paddingTop: insets.top + 12 }]}>
                <TouchableOpacity onPress={() => { if (currentRoom) studyRoomService.leaveRoom(currentRoom.id); setScreenState('lobby'); }}>
                    <ArrowLeft size={24} color={theme.text.primary} />
                </TouchableOpacity>
                <Text style={[styles.simpleHeaderTitle, { color: theme.text.primary }]} numberOfLines={1}>{currentRoom?.topic}</Text>
            ...
            <View style={styles.participantBar}>
                {participants.map(p => (
                    <View key={p.userId} style={[styles.participantChip, { backgroundColor: theme.background.secondary }]}>
                        {p.userId === currentRoom?.hostId && <Crown size={10} color="#FFD700" />}
                        <Text style={[styles.participantName, { color: theme.text.primary }]}>{p.displayName.substring(0, 8)}</Text>
                    </View>
                ))}
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={[styles.msgBubble, item.type === 'system' && styles.msgSystem,
                    item.userId === userId ? styles.msgSelf : styles.msgOther,
                    { backgroundColor: item.type === 'system' ? 'transparent' : (item.userId === userId ? 'rgba(26,188,156,0.15)' : theme.background.secondary) }
                    ]}>
                        {item.type !== 'system' && item.userId !== userId && (
                            <Text style={[styles.msgAuthor, { color: '#1ABC9C' }]}>{item.displayName}</Text>
                        )}
                        <Text style={[styles.msgText, { color: item.type === 'system' ? theme.text.secondary : theme.text.primary }]}>
                            {item.text}
                        </Text>
                    </View>
                )}
            ...
            {/* Input */}
            <View style={[styles.inputBar, { paddingBottom: insets.bottom + 8, backgroundColor: theme.background.secondary }]}>
                <TextInput
                    value={msgInput}
...
const styles = StyleSheet.create({
    ...
    createBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#1ABC9C', borderRadius: 14, paddingVertical: 12, paddingHorizontal: 24, marginTop: 8 },
    ...
    roomPlayerBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(26,188,156,0.12)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
    ...
    catChip: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 12, borderWidth: 1, borderColor: '#333' },
    catChipText: { fontSize: 13, fontWeight: '600' },
    createRoomBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#1ABC9C', borderRadius: 16, padding: 16, marginTop: 24 },
    createRoomBtnText: { color: '#FFF', fontSize: 16, fontWeight: '700' },

    participantBar: { flexDirection: 'row', paddingHorizontal: 16, gap: 6, flexWrap: 'wrap', paddingBottom: 8 },
    participantChip: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12 },
    ...
    inputBar: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingTop: 8, gap: 8 },
    msgInput: { flex: 1, height: 40, borderRadius: 20, paddingHorizontal: 16, backgroundColor: 'rgba(255,255,255,0.05)', fontSize: 14 },
    sendBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: '#1ABC9C', alignItems: 'center', justifyContent: 'center' },
});
