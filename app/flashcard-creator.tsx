...
import { StyleSheet, View, Text, TouchableOpacity, FlatList, TextInput, ActivityIndicator, Modal, Dimensions } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { ArrowLeft, Plus, Sparkles, Trash2, RotateCcw, CheckCircle, XCircle, BookOpen, Layers, Edit3 } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import Animated, { FadeInDown, ZoomIn, useSharedValue, useAnimatedStyle, withTiming, interpolate } from 'react-native-reanimated';
import { useTheme } from '../src/context/ThemeContext';
import { supabase } from '../src/config/supabase';
import { flashcardCreatorService, FlashcardDeck, Flashcard } from '../src/services/FlashcardCreatorService';
...
export default function FlashcardCreatorScreen() {
    ...
    const handleCreateDeck = async () => {
        ...
        const deck = await flashcardCreatorService.createDeck(userId, newDeckTitle.trim(), newDeckCategory.trim() || 'general');
    ...
    const handleAddCard = async () => {
        ...
        await flashcardCreatorService.addCard(userId, selectedDeck.id, newCardFront.trim(), newCardBack.trim());
    ...
    const startStudy = async (deck: FlashcardDeck) => {
        ...
        if (due.length === 0) { modal.info(i18n.t('flashcard.completed'), i18n.t('flashcard.all_studied')); return; }
        setDueCards(due);
        setCurrentCardIdx(0);
        setShowBack(false);
    ...
    const handleDeleteDeck = async (deckId: string) => {
        modal.confirm(i18n.t('common.delete'), 'Bu desteyi silmek istediğinize emin misiniz?',
            async () => {
    ...
    if (loading) {
        return <View style={[styles.container, styles.center, { backgroundColor: theme.background.primary }]}>
        ...
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                <View style={[styles.simpleHeader, { paddingTop: insets.top + 12 }]}>
                    ...
                    <Text style={[styles.simpleTitle, { color: theme.text.primary }]}>{currentCardIdx + 1}/{dueCards.length}</Text>
                    <View style={{ width: 24 }} />
                </View>

                <View style={styles.studyArea}>
                    <TouchableOpacity onPress={handleFlip} activeOpacity={0.9} style={styles.cardTouchable}>
                        <Animated.View style={[styles.flashcardFace, { backgroundColor: theme.background.secondary }, frontStyle]}>
                            <Text style={[styles.flashcardSide, { color: '#9B59B6' }]}>ÖN</Text>
                            <Text style={[styles.flashcardText, { color: theme.text.primary }]}>{card.front}</Text>
                            <Text style={[styles.tapHint, { color: theme.text.secondary }]}>Çevirmek için dokun</Text>
                        </Animated.View>
                        <Animated.View style={[styles.flashcardFace, styles.flashcardBack, { backgroundColor: theme.background.secondary }, backStyle]}>
                ...
                {showBack && (
                    <Animated.View entering={FadeInDown} style={styles.reviewBtns}>
                        <TouchableOpacity style={[styles.reviewBtn, { backgroundColor: '#FF4B4B' }]} onPress={() => handleReview(false)}>
                            <XCircle size={24} color="#FFF" />
                            <Text style={styles.reviewBtnText}>Tekrar</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.reviewBtn, { backgroundColor: '#2ECC71' }]} onPress={() => handleReview(true)}>
        ...
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                ...
                <View style={styles.form}>
                    ...
                    <TextInput value={newDeckTitle} onChangeText={setNewDeckTitle} placeholder="Ör: Fizik Formülleri"
                        placeholderTextColor="#666" style={[styles.formInput, { backgroundColor: theme.background.secondary, color: theme.text.primary }]} />
                    <Text style={[styles.formLabel, { color: theme.text.secondary, marginTop: 16 }]}>Kategori</Text>
                    <TextInput value={newDeckCategory} onChangeText={setNewDeckCategory} placeholder="Ör: science"
                        placeholderTextColor="#666" style={[styles.formInput, { backgroundColor: theme.background.secondary, color: theme.text.primary }]} />
    ...
    if (screenState === 'cards' && selectedDeck) {
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                <View style={[styles.simpleHeader, { paddingTop: insets.top + 12 }]}>
                    <TouchableOpacity onPress={() => { setScreenState('decks'); setSelectedDeck(null); }}>
                        <ArrowLeft size={24} color={theme.text.primary} />
                    </TouchableOpacity>
                    <Text style={[styles.simpleTitle, { color: theme.text.primary }]} numberOfLines={1}>{selectedDeck.title}</Text>
                        ...
                        <Animated.View entering={FadeInDown.delay(index * 50)}>
                            <View style={[styles.cardRow, { backgroundColor: theme.background.secondary }]}>
                                <View style={styles.cardContent}>
                                    <Text style={[styles.cardFront, { color: theme.text.primary }]} numberOfLines={1}>{item.front}</Text>
                                    <Text style={[styles.cardBackText, { color: theme.text.secondary }]} numberOfLines={1}>{item.back}</Text>
                                </View>
                                <View style={[styles.boxBadge, { backgroundColor: item.box >= 4 ? '#2ECC7120' : '#FF6B0020' }]}>
                                    <Text style={{ color: item.box >= 4 ? '#2ECC71' : '#FF6B00', fontSize: 11, fontWeight: '700' }}>Box {item.box}</Text>
                                </View>
                                {item.createdBy === 'ai' && <Sparkles size={14} color="#FFD700" />}
                            </View>
                        </Animated.View>
                    )}
                    ListEmptyComponent={<Text style={[styles.emptyText, { color: theme.text.secondary }]}>Henüz kart yok. Ekle veya AI ile oluştur!</Text>}
                />
                <CustomModal {...modal.modalProps} />
            </View>
        );
    ...
    if (screenState === 'add-card' && selectedDeck) {
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                ...
                <View style={styles.form}>
                    <Text style={[styles.formLabel, { color: theme.text.secondary }]}>Ön Yüz (Soru / Terim)</Text>
                    <TextInput value={newCardFront} onChangeText={setNewCardFront} placeholder="Ör: Newton'un 2. Yasası"
                        placeholderTextColor="#666" multiline style={[styles.formInput, styles.formInputMulti, { backgroundColor: theme.background.secondary, color: theme.text.primary }]} />
                    <Text style={[styles.formLabel, { color: theme.text.secondary, marginTop: 12 }]}>Arka Yüz (Cevap / Tanım)</Text>
                    <TextInput value={newCardBack} onChangeText={setNewCardBack} placeholder="Ör: F = m × a"
                        placeholderTextColor="#666" multiline style={[styles.formInput, styles.formInputMulti, { backgroundColor: theme.background.secondary, color: theme.text.primary }]} />
                    <TouchableOpacity style={[styles.createDeckBtn, { marginTop: 12 }]} onPress={handleAddCard}>
                        <Plus size={18} color="#FFF" />
                        <Text style={styles.createDeckBtnText}>Kart Ekle</Text>
                    </TouchableOpacity>

                    <View style={styles.aiSection}>
                        <Text style={[styles.aiTitle, { color: theme.text.primary }]}>🤖 AI ile Otomatik Oluştur</Text>
                        <TextInput value={aiTopic} onChangeText={setAiTopic} placeholder="Konu girin: Ör: Kimyasal bağlar"
                            placeholderTextColor="#666" style={[styles.formInput, { backgroundColor: theme.background.secondary, color: theme.text.primary }]} />
                        <TouchableOpacity style={styles.aiBtn} onPress={handleAIGenerate} disabled={aiGenerating}>
                            {aiGenerating ? <ActivityIndicator size="small" color="#FFD700" /> : <Sparkles size={18} color="#FFD700" />}
                            <Text style={styles.aiBtnText}>{aiGenerating ? i18n.t('flashcard.creating') : i18n.t('flashcard.create_cards')}</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            </View>
            ...
            <FlatList
                ...
                renderItem={({ item, index }) => (
                    <Animated.View entering={FadeInDown.delay(index * 80)}>
                        <TouchableOpacity style={[styles.deckCard, { backgroundColor: theme.background.secondary }]}
                            onPress={() => { setSelectedDeck(item); setScreenState('cards'); }}
                ...
                ListEmptyComponent={
                    <View style={styles.emptyView}>
                        ...
                        <TouchableOpacity style={styles.createDeckBtn} onPress={() => setScreenState('create-deck')}>
                            <Plus size={18} color="#FFF" />
...
const styles = StyleSheet.create({
    ...
    simpleHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingBottom: 12 },
    ...
    studyBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#9B59B6', borderRadius: 14, padding: 14 },
    ...
    createDeckBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#9B59B6', borderRadius: 14, padding: 14, marginTop: 16 },
    createDeckBtnText: { color: '#FFF', fontSize: 15, fontWeight: '700' },

    aiSection: { marginTop: 24, gap: 10 },
    aiTitle: { fontSize: 15, fontWeight: '700' },
    aiBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: 'rgba(255,215,0,0.1)', borderRadius: 14, padding: 14, borderWidth: 1, borderColor: 'rgba(255,215,0,0.3)', marginTop: 4 },
    ...
    flashcardFace: { position: 'absolute', width: '100%', height: '100%', borderRadius: 24, padding: 24, justifyContent: 'center', alignItems: 'center', backfaceVisibility: 'hidden' },
    flashcardBack: {},
    ...
    reviewBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 16, padding: 16 },
