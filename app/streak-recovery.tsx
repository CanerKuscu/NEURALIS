...
import { ArrowLeft, Flame, Clock, Target, CheckCircle, Circle, AlertTriangle, Star, Zap } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import Animated, { FadeInDown, ZoomIn, useSharedValue, useAnimatedStyle, withRepeat, withTiming, withSequence } from 'react-native-reanimated';
import { useTheme } from '../src/context/ThemeContext';
import { ShadowFox } from '../src/components';
import { supabase } from '../src/config/supabase';
import { streakRecoveryService, StreakRecoveryChallenge } from '../src/services/StreakRecoveryService';
...
export default function StreakRecoveryScreen() {
    ...
    const loadChallenge = async () => {
        try {
            ...
            if (!c && params.streak) {
                c = await streakRecoveryService.initiateRecovery(session.user.id, parseInt(params.streak) || 0);
            }
            if (c) {
                setChallenge(c);
    ...
    if (!challenge) {
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary, alignItems: 'center', justifyContent: 'center' }]}>
                <Text style={{ color: theme.text.secondary, fontSize: 16 }}>Aktif kurtarma mücadelesi yok</Text>
                <TouchableOpacity onPress={() => router.back()} style={styles.backLink}>
                    <Text style={{ color: '#FF6B00', fontSize: 14, fontWeight: '600' }}>{i18n.t('streak.go_back')}</Text>
                </TouchableOpacity>
            </View>
        );
    }
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            {/* Header */}
            <LinearGradient colors={isCompleted ? ['#2ECC71', '#27AE60'] : ['#FF4B4B', '#E74C3C']} style={[styles.header, { paddingTop: insets.top + 8 }]}>
                ...
                <Animated.View style={[styles.streakBigCircle, pulseStyle]}>
                    <ShadowFox state={isCompleted ? 'healthy' : 'critical'} mood={isCompleted ? 'happy' : 'thinking'} size="small" showGlow={false} style={{ width: 60, height: 60 }} />
            ...
            {/* Timer */}
            {!isCompleted && (
                <Animated.View entering={FadeInDown.delay(200)} style={[styles.timerCard, { backgroundColor: theme.background.secondary }]}>
            ...
            <View style={styles.reqContainer}>
                ...
                <Animated.View entering={FadeInDown.delay(300)} style={[styles.reqItem, { backgroundColor: theme.background.secondary }]}>
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <View style={styles.reqProgress}>
                            <View style={[styles.reqProgressFill, { width: `${(challenge.requirements.lessonsCompleted / challenge.requirements.lessonsNeeded) * 100}%` }]} />
                        </View>
                        <Text style={[styles.reqItemSub, { color: theme.text.secondary }]}>
                            {challenge.requirements.lessonsCompleted}/{challenge.requirements.lessonsNeeded} tamamlandı
                        </Text>
                    </View>
                </Animated.View>

                <Animated.View entering={FadeInDown.delay(400)} style={[styles.reqItem, { backgroundColor: theme.background.secondary }]}>
                    ...
                    <View style={{ flex: 1 }}>
                        ...
                        <Text style={[styles.reqItemSub, { color: theme.text.secondary }]}>
                            {challenge.requirements.perfectScoreAchieved ? i18n.t('streak.success') : i18n.t('streak.perfect_accuracy')}
                        </Text>
                    </View>
                </Animated.View>
            </View>
            ...
            {!isCompleted ? (
                ...
                <Animated.View entering={ZoomIn} style={styles.successContainer}>
                    ...
                    <TouchableOpacity style={[styles.startBtn, { backgroundColor: '#2ECC71' }]} onPress={() => router.replace('/(tabs)')}>
...
const styles = StyleSheet.create({
    container: { flex: 1 },
    header: { alignItems: 'center', paddingBottom: 24, borderBottomLeftRadius: 30, borderBottomRightRadius: 30 },
    headerRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', paddingHorizontal: 16, marginBottom: 16 },
    backBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
    headerTitle: { fontSize: 18, fontWeight: '800', color: '#FFF' },
    streakBigCircle: { width: 90, height: 90, borderRadius: 45, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
    ...
    timerCard: { flexDirection: 'row', alignItems: 'center', gap: 8, marginHorizontal: 16, marginTop: 16, padding: 14, borderRadius: 14 },
    ...
    startBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#FF4B4B', borderRadius: 16, padding: 18 },
