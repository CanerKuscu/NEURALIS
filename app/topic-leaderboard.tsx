...
import { StyleSheet, View, Text, TouchableOpacity, FlatList, ActivityIndicator, Dimensions } from 'react-native';
import { useSafeAreaInsets } from 'react-native-safe-area-context';
import { useRouter } from 'expo-router';
import { ArrowLeft, Trophy, Medal, Crown, TrendingUp, ChevronDown } from 'lucide-react-native';
...
export default function TopicLeaderboardScreen() {
    ...
    useEffect(() => { loadLeaderboard(); }, [selectedCat]);
    ...
    const loadLeaderboard = async () => {
        ...
        try {
            ...
            if (selectedCat === 'all') {
                ...
                const { data: d } = await supabase.from('user_category_levels')
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            {/* Header */}
            <LinearGradient colors={[catColor, catColor + 'CC']} style={[styles.header, { paddingTop: insets.top + 8 }]}>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <TouchableOpacity
                        onPress={() => { setSelectedCat(item.id); Haptics.selectionAsync(); }}
                        style={[styles.catTab, selectedCat === item.id && { backgroundColor: item.color + '25', borderColor: item.color }]}
                    >
            ...
            {loading ? (
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <Text style={[styles.emptyText, { color: theme.text.secondary }]}>Henüz sıralama verisi yok</Text>
                    }
                />
            )}
        </View>
...
const styles = StyleSheet.create({
    ...
    myRankCard: { alignItems: 'center', marginTop: 12, backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: 14, padding: 12 },
    ...
    catTab: { paddingHorizontal: 14, paddingVertical: 6, borderRadius: 16, borderWidth: 1, borderColor: '#333' },
    catTabText: { fontSize: 13, fontWeight: '600', color: '#888' },

    entryRow: { flexDirection: 'row', alignItems: 'center', borderRadius: 14, padding: 14, marginBottom: 8 },
