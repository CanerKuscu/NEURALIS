...
import { ArrowLeft, TrendingUp, TrendingDown, Flame, Target, BookOpen, Zap, Clock, Award, ChevronRight } from 'lucide-react-native';
import { LinearGradient } from 'expo-linear-gradient';
import Animated, { FadeInDown, FadeIn, ZoomIn } from 'react-native-reanimated';
import { useTheme } from '../src/context/ThemeContext';
import { supabase } from '../src/config/supabase';
import { weeklySummaryService, WeeklySummary } from '../src/services/WeeklySummaryService';
...
export default function WeeklySummaryScreen() {
    ...
    if (loading) {
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                ...
                <Text style={[styles.loadingText, { color: theme.text.secondary }]}>🦊 Raporun hazırlanıyor...</Text>
            </View>
        );
    }
    ...
    return (
        <ScrollView style={[styles.container, { backgroundColor: theme.background.primary }]}>
            {/* Header + Fox message */}
            <LinearGradient colors={['#2C3E50', '#34495E']} style={[styles.header, { paddingTop: insets.top + 8 }]}>
            ...
            <View style={styles.statsGrid}>
                <Animated.View entering={FadeInDown.delay(300)} style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                    <BookOpen size={24} color="#3498DB" />
                    <Text style={[styles.statValue, { color: theme.text.primary }]}>{summary.stats.lessonsCompleted}</Text>
                    <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('weekly.lessons')}</Text>
                    <View style={styles.changeRow}>
                        {changeIcon(summary.comparison.lessonsChange)}
                        <Text style={[styles.changeText, { color: changeColor(summary.comparison.lessonsChange) }]}>
                            {changeText(summary.comparison.lessonsChange, '')}
                        </Text>
                    </View>
                </Animated.View>

                <Animated.View entering={FadeInDown.delay(350)} style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                    <Zap size={24} color="#FFD700" />
                    <Text style={[styles.statValue, { color: theme.text.primary }]}>{summary.stats.xpEarned}</Text>
                ...
                <Animated.View entering={FadeInDown.delay(400)} style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                    <Flame size={24} color="#FF6B00" />
                    <Text style={[styles.statValue, { color: theme.text.primary }]}>{summary.stats.streakDays}</Text>
                    <Text style={[styles.statLabel, { color: theme.text.secondary }]}>Streak</Text>
                </Animated.View>

                <Animated.View entering={FadeInDown.delay(450)} style={[styles.statCard, { backgroundColor: theme.background.secondary }]}>
                    <Target size={24} color="#2ECC71" />
                    <Text style={[styles.statValue, { color: theme.text.primary }]}>%{summary.stats.accuracy}</Text>
                    <Text style={[styles.statLabel, { color: theme.text.secondary }]}>{i18n.t('weekly.accuracy')}</Text>
                    <View style={styles.changeRow}>
                        {changeIcon(summary.comparison.accuracyChange)}
                        <Text style={[styles.changeText, { color: changeColor(summary.comparison.accuracyChange) }]}>
            ...
            {/* Time + Quests */}
            <Animated.View entering={FadeInDown.delay(500)} style={[styles.timeRow, { backgroundColor: theme.background.secondary }]}>
                <View style={styles.timeItem}>
                    <Clock size={18} color="#9B59B6" />
                    <Text style={[styles.timeValue, { color: theme.text.primary }]}>{summary.stats.timeSpentMinutes} dk</Text>
                    <Text style={[styles.timeLabel, { color: theme.text.secondary }]}>{i18n.t('weekly.total_time')}</Text>
                </View>
                <View style={styles.timeDivider} />
                <View style={styles.timeItem}>
                    <Award size={18} color="#E67E22" />
                    <Text style={[styles.timeValue, { color: theme.text.primary }]}>{summary.stats.questsCompleted}</Text>
            ...
            <View style={styles.section}>
                ...
                <View style={[styles.catCompare, { backgroundColor: theme.background.secondary }]}>
                    <View style={styles.catItem}>
                        ...
                        <Text style={[styles.catCategory, { color: theme.text.primary }]}>{summary.topCategory}</Text>
                    </View>
                    ...
                    <View style={styles.catItem}>
                        ...
                        <Text style={[styles.catCategory, { color: theme.text.primary }]}>{summary.weakestCategory}</Text>
                    </View>
                </View>
            </View>
            ...
            <View style={styles.section}>
                ...
                {summary.improvements.map((imp, i) => (
                    <View key={i} style={[styles.improvementItem, { backgroundColor: theme.background.secondary }]}>
            ...
            {summary.badgesEarned.length > 0 && (
                <View style={styles.section}>
                    <Text style={[styles.sectionTitle, { color: theme.text.primary }]}>{i18n.t('weekly.badges_earned')}</Text>
...
const styles = StyleSheet.create({
    ...
    foxCard: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: 'rgba(255,255,255,0.1)', borderRadius: 16, padding: 14, marginTop: 16 },
    ...
    statCard: { width: (width - 42) / 2, borderRadius: 16, padding: 14, alignItems: 'center', gap: 4 },
    ...
    improvementItem: { flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: 12, padding: 12 },
