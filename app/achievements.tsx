...
import { ArrowLeft, Trophy, Flame, Zap, Target, BookOpen, Share2, Lock, Star } from 'lucide-react-native';
...
const ProgressBar = ({ progress, color, style }: { progress: number; color: string; style?: any }) => {
    return (
        <View style={[styles.progressTrack, style]}>
            <View style={[styles.progressBar, { width: `${Math.min(100, progress * 100)}%`, backgroundColor: color }]} />
        </View>
    );
};
...
export default function AchievementsScreen() {
    ...
    const [achievements, setAchievements] = useState<Achievement[]>(DEFINITIONS.map(d => ({ ...d, current: 0, unlocked: false })));
        ...
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            ...
            >
                    ...
                    >
                        <View style={styles.summaryContent}>
                            ...
                            <View style={styles.summaryText}>
                                <Text style={styles.summaryTitle}>{totalUnlocked} / {achievements.length} Unlocked</Text>
                ...
                <Animated.View entering={FadeInDown.delay(150)}>
                    <View style={[styles.showcaseSection, { backgroundColor: theme.background.secondary, borderColor: theme.border.light }]}>
                        <View style={styles.showcaseHeader}>
                            <Star size={18} color="#F1C40F" />
                            <Text style={[styles.showcaseTitle, { color: theme.text.primary }]}>Vitrin Rozetleri</Text>
                            <Text style={[styles.showcaseCount, { color: theme.text.tertiary }]}>{showcaseBadges.length}/3</Text>
                        </View>
                        ...
                        <View style={styles.showcaseSlots}>
                            {[0, 1, 2].map(i => {
                                ...
                                return (
                                    <View key={i} style={[styles.showcaseSlot, { backgroundColor: badge ? `${badge.color}20` : theme.background.tertiary, borderColor: badge ? badge.color : theme.border.light }]}>
                                        {badge ? (
                                            <>
                                                {badge.icon}
                                                <Text style={[styles.showcaseSlotText, { color: badge.color }]}>{badge.title}</Text>
                                            </>
                                        ) : (
                                            <Text style={[styles.showcaseSlotEmpty, { color: theme.text.muted }]}>Boş</Text>
                                        )}
                                    </View>
                                );
                            })}
                    ...
                    {achievements.map((item, index) => (
                        ...
                        >
                            <View style={styles.cardHeader}>
                                <View style={[styles.iconBox, { backgroundColor: item.unlocked ? adjustColor(item.color, 150) : theme.background.tertiary }]}>
                                    {item.unlocked ? item.icon : <Lock size={24} color={theme.text.tertiary} />}
                                </View>
                                <View style={styles.cardContent}>
                                    <Text style={[styles.cardTitle, { color: theme.text.primary, opacity: item.unlocked ? 1 : 0.6 }]}>
                                        {item.title}
                                    </Text>
                                ...
                                {item.unlocked && (
                                    <View style={{ flexDirection: 'row', gap: 8 }}>
                                        <TouchableOpacity onPress={() => toggleShowcase(item.id)}>
                                            <View style={[styles.showcaseToggle, { backgroundColor: showcaseBadges.includes(item.id) ? item.color : theme.background.tertiary }]}>
                                                <Star size={14} color={showcaseBadges.includes(item.id) ? '#FFF' : theme.text.tertiary} />
                                            </View>
                                        </TouchableOpacity>
                                        <View style={[styles.xpBadge, { backgroundColor: theme.background.tertiary }]}>
                                            <Text style={[styles.xpText, { color: theme.primary }]}>+{item.xpReward} XP</Text>
                                        </View>
                                    </View>
                                )}
                            </View>
                            ...
                            <View style={styles.progressContainer}>
                                <View style={styles.progressLabelRow}>
                                    <Text style={[styles.progressLabel, { color: item.unlocked ? item.color : theme.text.secondary }]}>
...
const styles = StyleSheet.create({
    ...
    showcaseSlot: {
        flex: 1, height: 80, borderRadius: 16, borderWidth: 1.5, borderStyle: 'dashed',
