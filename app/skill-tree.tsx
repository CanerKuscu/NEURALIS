...
import { skillTreeService, SkillTree, SkillNode } from '../src/services/SkillTreeService';
...
export default function SkillTreeScreen() {
    ...
    const handleStartNode = () => {
        ...
        router.push(`/lesson?category=${tree.category}&topic=${selectedNode.title}&skillNode=${selectedNode.id}`);
    };
    ...
    if (!selectedCategory) {
        return (
            <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
                <LinearGradient colors={['#2C3E50', '#3D566E']} style={[styles.header, { paddingTop: insets.top + 8 }]}>
                ...
                <View style={styles.catList}>
                    {categories.map((cat, i) => (
                        <Animated.View key={cat.id} entering={FadeInDown.delay(i * 100)}>
                            <TouchableOpacity style={[styles.catCard, { backgroundColor: theme.background.secondary }]}
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background.primary }]}>
            ...
            <ScrollView style={{ flex: 1 }} contentContainerStyle={{ height: TREE_HEIGHT, paddingHorizontal: 16 }}>
                {/* Connection Lines */}
                        ...
                        node.prerequisites.map(preId => {
                            ...
                            return (
                                <Line key={`${preId}-${node.id}`}
                                    ...
                                    stroke={color} strokeWidth={2} strokeDasharray={node.status === 'locked' ? '6,4' : undefined} />
                            );
                        })
                    )}
                </Svg>
                    ...
                    return (
                        <Animated.View key={node.id}
                            ...
                            <TouchableOpacity
                                ...
                                style={[
                                    styles.nodeCircle,
                                    { borderColor: color, backgroundColor: node.status === 'completed' ? color : 'rgba(0,0,0,0.6)' },
                            ...
                            <Text style={[styles.nodeLabel, { color: node.status === 'locked' ? '#555' : theme.text.primary }]} numberOfLines={1}>
                                {node.title}
                            </Text>
            ...
            {selectedNode && (
                <Animated.View entering={FadeInDown} style={[styles.nodeDetail, { backgroundColor: theme.background.secondary }]}>
                    <View style={styles.nodeDetailHeader}>
                        <Text style={styles.nodeDetailIcon}>{selectedNode.icon}</Text>
                        <View style={{ flex: 1 }}>
                            <Text style={[styles.nodeDetailTitle, { color: theme.text.primary }]}>{selectedNode.title}</Text>
                            <Text style={[styles.nodeDetailDesc, { color: theme.text.secondary }]}>{selectedNode.description}</Text>
                        </View>
                    ...
                    {selectedNode.status !== 'completed' && (
                        <TouchableOpacity style={styles.startNodeBtn} onPress={handleStartNode}>
                            ...
                            <Text style={styles.startNodeBtnText}>
                                {selectedNode.status === 'in-progress' ? i18n.t('skill_tree_screen.continue_btn') : i18n.t('skill_tree_screen.start_btn')}
                            </Text>
                        </TouchableOpacity>
                    )}
                </Animated.View>
...
const styles = StyleSheet.create({
    ...
    treeHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingBottom: 8 },
    treeTitle: { fontSize: 16, fontWeight: '700' },
    treePct: { fontSize: 15, fontWeight: '800' },

    progressBar: { height: 4, backgroundColor: '#222', marginHorizontal: 16, borderRadius: 2, marginBottom: 8 },
    ...
    nodeCircle: { width: NODE_SIZE, height: NODE_SIZE, borderRadius: NODE_SIZE / 2, borderWidth: 2, alignItems: 'center', justifyContent: 'center' },
    nodeIcon: { fontSize: 22 },
    nodeLabel: { fontSize: 10, fontWeight: '700', marginTop: 4, textAlign: 'center' },
    progressRing: { position: 'absolute', top: -4, right: -4, width: 22, height: 22, borderRadius: 11, borderWidth: 2, backgroundColor: '#1A1A2E', alignItems: 'center', justifyContent: 'center' },
    progressRingText: { fontSize: 7, fontWeight: '800', color: '#F39C12' },

    nodeDetail: { position: 'absolute', bottom: 0, left: 0, right: 0, borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: 20, paddingBottom: 36, gap: 12 },
    ...
    detailBadge: { flexDirection: 'row', alignItems: 'center', gap: 4, backgroundColor: 'rgba(255,255,255,0.06)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 10 },
    detailBadgeText: { color: '#CCC', fontSize: 12, fontWeight: '600' },
    startNodeBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: '#2ECC71', borderRadius: 16, padding: 16 },
