            ...
            <Animated.View entering={FadeInDown.delay(i * 60)}>
              ...
              >
                <View
                  style={[
                    styles.chatAvatar,
                    { backgroundColor: chat.type === 'group' ? '#9B59B620' : '#3498DB20' },
                  ]}
                >
                  {chat.type === 'group' ? (
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {chat.lastMessage && (
                    <Text
                      style={[styles.chatLastMsg, { color: theme.text.secondary }]}
                      numberOfLines={1}
                    >
        ...
        <View style={{ flex: 1, marginLeft: 12 }}>
          ...
          <Text style={[styles.chatHeaderStatus, { color: theme.text.secondary }]}>
            {activeChat?.type === 'group'
              ? `${(activeChat.participants?.length || 0) + 1} ${i18n.t('chat_screen.members')}`
              : i18n.t('chat_screen.online')}
          </Text>
        </View>
      </View>
          ...
          renderItem={({ item: msg }) => {
            ...
            return (
              <View style={[styles.msgRow, { justifyContent: isMe ? 'flex-end' : 'flex-start' }]}>
                ...
                >
                  {!isMe && (
                    <Text style={[styles.msgSender, { color: '#3498DB' }]}>{msg.senderName}</Text>
                  )}
                  <Text
                    style={[
                      styles.msgText,
                      {
                        ...
                        fontSize: msg.type === 'sticker' ? 32 : 15,
                      },
                    ]}
                  >
                    {msg.text}
                  ...
                  <Text
                    style={[
                      styles.msgTime,
                      { color: isMe ? 'rgba(255,255,255,0.6)' : theme.text.secondary },
                    ]}
                  >
          ...
          <View style={[styles.stickerPanel, { backgroundColor: theme.background.secondary }]}>
            {STICKER_PACKS.map((pack) => (
              <View key={pack.category}>
                <Text style={[styles.stickerCategory, { color: theme.text.secondary }]}>
                  {pack.category}
                </Text>
