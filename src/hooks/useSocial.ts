...
import { friendRequestDocToModel, synapseLinkDocToModel, userDocumentToProfile } from '../services/userHelpers';

const { db: database, doc, COLLECTIONS, collection, query, where, getDocs, getDoc, addDoc, writeBatch, updateDoc, deleteDoc, serverTimestamp, Timestamp, orderBy } = db as any;
...
export function useSocial(): UseSocialReturn {
    ...
    const createSynapseLink = useCallback(
        async (friendUid: string): Promise<void> => {
            ...
            try {
                ...
                const errorMessage = error instanceof Error ? error.message : 'Failed to create Synapse Link';
                setState(prev => ({ ...prev, error: errorMessage, isLoading: false }));
                throw error;
            }
        },
    ...
    const dissolveSynapseLink = useCallback(async (): Promise<void> => {
        ...
        try {
            ...
            const partnerUid = state.synapseLink.userAId === currentUserId
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to dissolve Synapse Link';
            setState(prev => ({ ...prev, error: errorMessage, isLoading: false }));
            throw error;
        }
    }, [currentUserId, state.synapseLink]);
    ...
    const syncLinkedStreak = useCallback(
        async (friendUid: string): Promise<void> => {
            ...
            try {
                ...
                const newSharedStreak = Math.min(
                ...
                batch.update(synapseLinkRef, {
                    userAStreak: state.synapseLink.userAId === currentUserId
                        ? currentUserData.streakCount
                        : friendData.streakCount,
                    userBStreak: state.synapseLink.userBId === currentUserId
                        ? currentUserData.streakCount
                        : friendData.streakCount,
