...
import type { NotificationLocale, ShadowFoxStatus } from '../services/LocalizationService';
import { localizationService } from '../services/LocalizationService';
import type { WidgetData } from '../services/WidgetService';
...
export function useNotifications(options: UseNotificationsOptions = {}): UseNotificationsReturn {
  ...
  const scheduleNotifications = useCallback(
    async (status: StreakStatus): Promise<void> => {
      ...
      try {
        ...
        const foxStatus = notificationEngine.getShadowFoxStatus(
          remainingMs,
          status.taskCompletedToday,
        );
  ...
  const updateWidget = useCallback(
    async (data: Omit<WidgetData, 'lastUpdated' | 'locale'>): Promise<void> => {
      try {
  ...
  const getShadowFoxStatus = useCallback(
    (remainingMs: number, taskCompleted: boolean): ShadowFoxStatus => {
