...
import { StreakState, StreakData, EnergyData } from '../types';
...
export function useNeuralisCore(): UseNeuralisCoreReturn {
    ...
    const { user, streakData, streakState, timeUntilDeadline, energyData, uiOpacity, isGrayscale, isLoading } =
        useNeuralisStore(useShallow((s: StoreState) => ({
            user: s.user,
    ...
    const handleStreakUpdate = useCallback(
        (streak: StreakData | null, state: StreakState) => {
