...
import { NotificationSeverity, ShadowFoxStatus, localizationService } from '../services/LocalizationService';
import { notificationEngine, StreakStatus } from '../services/NotificationEngine';
...
export function useAudioDecay(options: UseAudioDecayOptions): UseAudioDecayReturn {
    ...
    const playDecaySound = useCallback(async (): Promise<void> => {
        ...
        try {
            const remainingMs = streakStatus
                ? streakStatus.deadlineTimestamp - Date.now()
                : Infinity;
    ...
    useEffect(() => {
        ...
        const updateDecayState = async (): Promise<void> => {
            ...
            const foxStatus: ShadowFoxStatus =
                phase.name === 'normal' ? 'happy' :
                    phase.name === 'urgent' ? 'tense' :
                        phase.name === 'decay' ? 'fading' :
                            phase.name === 'critical' ? 'critical' : 'dead';
            ...
            setState((prev) => ({
                ...
                nextScheduledAt: phase.intervalMs !== Infinity
                    ? now + phase.intervalMs
                    : null,
            }));
        ...
        return unsubscribe;
    }, [enabled, streakStatus, loadSound, onPhaseChange, onMonetizationTrigger, onStreakDeath, playDecaySound, triggerVibration]);
    ...
    const foxOpacity = useMemo(
        () => calculateFoxOpacity(state.remainingMs),
        [state.remainingMs]
    );
