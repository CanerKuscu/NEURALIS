...
import { StyleSheet, View, Text, TouchableOpacity, Modal, Dimensions, BackHandler } from 'react-native';
import Animated, {
    useSharedValue,
    useAnimatedStyle,
    withSpring,
...
const TYPE_CONFIG: Record<ModalType, { color: string; IconComponent: React.FC<any>; iconColor: string; bgColor: string }> = {
    confirm: { color: MODAL_COLORS.blue, IconComponent: Info, iconColor: MODAL_COLORS.blue, bgColor: 'rgba(28,176,246,0.15)' },
    info: { color: MODAL_COLORS.blue, IconComponent: Info, iconColor: MODAL_COLORS.blue, bgColor: 'rgba(28,176,246,0.15)' },
    error: { color: MODAL_COLORS.red, IconComponent: ShieldAlert, iconColor: MODAL_COLORS.red, bgColor: 'rgba(255,75,75,0.15)' },
    success: { color: MODAL_COLORS.primary, IconComponent: CheckCircle, iconColor: MODAL_COLORS.primary, bgColor: 'rgba(88,204,2,0.15)' },
    premium: { color: MODAL_COLORS.gold, IconComponent: Crown, iconColor: MODAL_COLORS.gold, bgColor: 'rgba(255,200,0,0.15)' },
    warning: { color: MODAL_COLORS.gold, IconComponent: AlertTriangle, iconColor: MODAL_COLORS.gold, bgColor: 'rgba(255,200,0,0.15)' },
};
...
export function CustomModal({
    ...
    const getButtonStyle = (btnStyle?: string) => {
        switch (btnStyle) {
            case 'cancel':
                return { backgroundColor: MODAL_COLORS.card, borderWidth: 1, borderColor: MODAL_COLORS.border };
            case 'destructive':
...
export function useModal() {
    ...
    const confirm = useCallback((title: string, message: string, onConfirm: () => void, confirmText = 'OK', cancelText?: string) => {
        show({
            type: 'confirm',
            title,
            message,
            buttons: [
                ...
                { text: confirmText, onPress: () => { hide(); onConfirm(); }, style: 'primary' },
            ],
        });
    }, [show, hide]);
    ...
    const info = useCallback((title: string, message?: string, onOk?: () => void) => {
        show({
            ...
            title,
            message,
            buttons: [{ text: 'OK', onPress: () => { hide(); onOk?.(); }, style: 'primary' }],
        });
    }, [show, hide]);
    ...
    const premium = useCallback((title: string, message: string, onUpgrade: () => void) => {
        show({
            type: 'premium',
            title,
            message,
            ...
            buttons: [
                ...
                { text: 'Premium', onPress: () => { hide(); onUpgrade(); }, style: 'primary' },
            ],
        });
    }, [show, hide]);
