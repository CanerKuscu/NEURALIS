...
import { LeagueParticipant } from '../types/leagueTypes';
...
const LeaguePodium: React.FC<LeaguePodiumProps> = ({ topThree }) => {
    ...
    useEffect(() => {
        ...
        Animated.stagger(150, [
            Animated.spring(scaleAnims[1], { toValue: 1, useNativeDriver: true, tension: 50, friction: 7 }),
            Animated.spring(scaleAnims[0], { toValue: 1, useNativeDriver: true, tension: 50, friction: 7 }),
            Animated.spring(scaleAnims[2], { toValue: 1, useNativeDriver: true, tension: 50, friction: 7 }),
        ]).start();
    }, []);
            ...
            {displayOrder.map((displayIndex, i) => {
                ...
                return (
                    <Animated.View
                        ...
                        style={[
                            styles.podiumItem,
                            { transform: [{ scale: scaleAnims[displayIndex] }] },
                        ]}
                    >
                        {/* Trophy */}
                        <View style={[styles.trophyContainer, { width: config.size, height: config.size * 1.2 }]}>
