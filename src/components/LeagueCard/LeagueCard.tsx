...
export const LeagueCard: React.FC<LeagueCardProps> = ({
  ...
  return (
    <TouchableOpacity style={[styles.container, style]} onPress={onPress} activeOpacity={0.8}>
      <LinearGradient colors={[COLORS.gray[900], COLORS.pureBlack]} style={styles.cardGradient}>
        {/* League badge */}
        <View style={styles.header}>
          <LinearGradient colors={getGradientColors()} style={styles.leagueBadge}>
        ...
        <View style={styles.statsRow}>
          <View style={styles.statItem}>
            <Text style={[styles.statValue, { color: config.color }]}>#{position}</Text>
          ...
          <View style={styles.statItem}>
            <Text style={styles.statValue}>{Math.round(rankPoints).toLocaleString()}</Text>
...
export const LeagueBadge: React.FC<LeagueBadgeProps> = ({
  ...
  return (
    <View style={badgeStyles.container}>
      ...
      >
        <Text style={[badgeStyles.icon, { fontSize: sizes[size].icon }]}>{config.icon}</Text>
      </LinearGradient>
      {showLabel && (
        <Text style={[badgeStyles.label, { color: config.color }]}>{config.displayName}</Text>
      )}
    </View>
  );
};
