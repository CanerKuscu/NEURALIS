...
import Svg, { G, Path, Circle, Ellipse, Defs, RadialGradient, LinearGradient, Stop } from 'react-native-svg';
import {
    AvatarConfig,
    ...
    const hairPaths: Record<HairType, React.ReactNode> = {
        ...
        medium: (
            <G>
                ...
                />
                <Path d="M48,80 Q45,95 50,108" stroke={color} strokeWidth="7" strokeLinecap="round" fill="none" />
                <Path d="M152,80 Q155,95 150,108" stroke={color} strokeWidth="7" strokeLinecap="round" fill="none" />
            </G>
        ),
        long: (
            <G>
...
const Mouth = ({ type }: { type: MouthType }) => {
    ...
    const mouthStyles: Record<MouthType, React.ReactNode> = {
        ...
        grin: (
            <G>
                <Path d={`M${mouthX - 18},${mouthY} Q${mouthX},${mouthY + 18} ${mouthX + 18},${mouthY}`} fill="#1A1A1A" />
                <Path d={`M${mouthX - 15},${mouthY} L${mouthX + 15},${mouthY} L${mouthX + 12},${mouthY + 6} L${mouthX - 12},${mouthY + 6} Z`} fill="#FFF" />
            </G>
        ),
        neutral: (
            <Path
...
const Shirt = ({ type, color, skinColor }: { type: ShirtType; color: string; skinColor: string }) => {
    ...
    return (
        <G>
            ...
            <Path d="M60,133 Q45,133 45,160 L42,200 L158,200 L155,160 Q155,133 140,133 Z" fill="url(#shirtGrad)" />

            {/* Style specific details */}
            {type === 'hoodie' && (
                <G>
                    <Path d="M65,133 Q100,128 135,133" stroke={darker} strokeWidth="8" strokeLinecap="round" fill="none" />
                    <Path d="M70,172 Q100,178 130,172 L128,192 Q100,198 72,192 Z" fill={darker} opacity={0.3} />
            ...
            {type === 'tank' && (
                <G>
                    ...
                    <Path d="M60,138 Q50,153 50,173" stroke={skinColor} strokeWidth="18" strokeLinecap="round" fill="none" />
                    <Path d="M140,138 Q150,153 150,173" stroke={skinColor} strokeWidth="18" strokeLinecap="round" fill="none" />
                </G>
            )}
...
export const AvatarView = React.memo(({ config = DEFAULT_AVATAR_CONFIG, size = 150, style, showBackground = true }: AvatarViewProps) => {
    // If config comes from potentially untyped/old source, default it safely
    const safeConfig = { ...DEFAULT_AVATAR_CONFIG, ...config };

    const {
        skinColor, hairType, hairColor, eyeType, eyeColor,
        mouthType, shirtType, shirtColor, bgColor
    } = safeConfig;
    ...
    return (
        <View style={[{ width: size, height: size }, style]}>
            <Svg width="100%" height="100%" viewBox="0 0 200 200">
                ...
                {/* Background - Optional */}
                {showBackground && (
                    <Circle cx="100" cy="100" r="95" fill="url(#bgGradient_main)" />
                )}
