...
export const SubscriptionProvider: React.FC<SubscriptionProviderProps> = ({ children, userId }) => {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const isElite =
          info.entitlements.active[REVENUECAT_CONFIG.ENTITLEMENTS.ELITE] !== undefined;
  ...
  const checkEntitlements = async (info?: CustomerInfo) => {
    try {
      ...
      const isPro =
        customerInfo.entitlements.active[REVENUECAT_CONFIG.ENTITLEMENTS.PRO] !== undefined;
      const isElite =
        customerInfo.entitlements.active[REVENUECAT_CONFIG.ENTITLEMENTS.ELITE] !== undefined;
      setState((prev) => ({
        ...prev,
        isPro,
        isElite,
  ...
  const purchasePackage = useCallback(async (pkg: PurchasesPackage): Promise<boolean> => {
    ...
    try {
      ...
      if (__DEV__) {
        ...
        const isPro =
          (customerInfo as any).entitlements?.active?.[REVENUECAT_CONFIG.ENTITLEMENTS.PRO] !==
          undefined;
        setState((prev) => ({
          ...prev,
          isPro,
  ...
  const restorePurchases = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (rcAppleKey || rcGoogleKey) {
        ...
        const isPro =
          (info as any).entitlements?.active?.[REVENUECAT_CONFIG.ENTITLEMENTS.PRO] !== undefined;
        setState((prev) => ({
          ...prev,
          isPro,
          isLoading: false,
  ...
  const getAvailablePackages = useCallback((): PurchasesPackage[] => {
    ...
    return (
      state.offerings?.availablePackages ||
      ([
        {
          identifier: 'monthly',
          packageType: 'MONTHLY',
...
export { REVENUECAT_CONFIG, PRICING, PRO_FEATURES, FREE_LIMITS };
