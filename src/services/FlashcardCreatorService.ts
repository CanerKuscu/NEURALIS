...
class FlashcardCreatorService {
    ...
    async createDeck(userId: string, title: string, category: string, description?: string): Promise<FlashcardDeck> {
    ...
    async addCard(userId: string, deckId: string, front: string, back: string, tags: string[] = []): Promise<Flashcard> {
    ...
    async generateAICards(userId: string, deckId: string, topic: string, count: number = 5): Promise<Flashcard[]> {
        ...
        } catch (e) { console.warn('AI card generation failed:', e); }
        return generated;
    }

    async reviewCard(userId: string, deckId: string, cardId: string, correct: boolean): Promise<void> {
        const decks = await this.getDecks(userId);
        const deck = decks.find(d => d.id === deckId);
        if (!deck) return;
    ...
    async deleteDeck(userId: string, deckId: string): Promise<void> {
        const decks = await this.getDecks(userId);
        await this.saveDecks(userId, decks.filter(d => d.id !== deckId));
    }
