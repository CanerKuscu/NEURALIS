...
export function setNotificationHandler(handler: any): void {
  ...
  getModule()
    .then((m) => m?.setNotificationHandler && m.setNotificationHandler(handler))
    .catch(() => {});
}
...
export function addNotificationReceivedListener(cb: any): { remove: () => void } {
  ...
  getModule()
    .then((m) => {
      if (m?.addNotificationReceivedListener) sub = m.addNotificationReceivedListener(cb);
    })
    .catch(() => {});
  return {
    remove: () => {
...
export function addNotificationResponseReceivedListener(cb: any): { remove: () => void } {
  ...
  getModule()
    .then((m) => {
      if (m?.addNotificationResponseReceivedListener)
        sub = m.addNotificationResponseReceivedListener(cb);
    })
    .catch(() => {});
  return {
    remove: () => {
