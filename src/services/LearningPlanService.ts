    ...
    try {
      ...
      for (const cat of categories) {
        ...
        if (daysSincePractice > 7 || lessonsCompleted < 3) {
          weakCategories.push({
            category: cat,
            errorRate: Math.min(0.8, daysSincePractice / 30),
            lastPracticed,
          });
  ...
  private buildPlan(userId: string, stats: PlanStats): WeeklyPlan {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      for (let i = 0; i < Math.min(dailyTarget, focusCategories.length + 1); i++) {
        const cat =
          ...
          stats.strongCategories[0]?.category ||
          'mathematics';
    ...
    const insights = [
      ...
      stats.currentStreak > 0
        ? `${stats.currentStreak} günlük serin var, devam et! 🔥`
        : 'Bu hafta yeni bir seri başlat! 💪',
    ];
