...
class AvatarService {
  ...
  async unlockItem(userId: string, itemId: string): Promise<boolean> {
    try {
      const { error } = await supabase.from('user_avatar_items').upsert({
        user_id: userId,
        item_id: itemId,
  ...
  async equipItem(userId: string, itemId: string, category: AvatarCategory): Promise<boolean> {
    try {
      ...
      const { error } = await supabase.from('user_avatar_state').upsert(updateData);
  ...
  async checkAndUnlockNewItems(userId: string): Promise<AvatarItem[]> {
    try {
      ...
      // Items not owned
      const { data: allItems } = await supabase.from('avatar_items').select('*');
  ...
  private async getDefaultItems(): Promise<AvatarItem[]> {
    const { data } = await supabase.from('avatar_items').select('*').eq('is_default', true);
    return (data || []).map(this.mapToAvatarItem);
  }
