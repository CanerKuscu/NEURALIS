...
class SpacedRepetitionService {
    ...
    /** Yeni kart ekle */
    async addCard(front: string, back: string, category: string, subcategory?: string, hint?: string): Promise<FlashCard> {
    ...
    async addCardsFromLesson(lessonData: { question: string; answer: string; category: string }[]): Promise<FlashCard[]> {
    ...
    async getReviewSession(): Promise<ReviewSession> {
        ...
        cards.forEach(c => { boxDistribution[c.box] = (boxDistribution[c.box] || 0) + 1; });
    ...
    async getCategoryStats(): Promise<Record<string, { total: number; mastered: number; due: number }>> {
        const cards = await this.getAllCards();
        const today = this.getToday();
