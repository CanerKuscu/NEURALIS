...
import { TaskCategory, TaskDifficulty, TaskAttempt } from '../types';
...
export class NeuralArchiveService {
     ...
     */
    async recordErrorBatch(errors: Omit<NeuralError, 'id' | 'timestamp' | 'isAnalyzed'>[]): Promise<string[]> {
    ...
    private buildWeaknessPattern(
        ...
        for (const error of errors) {
            ...
            if (daysSince <= 7) recentCount++; else if (daysSince <= 14) olderCount++;
        }
        ...
        const worstTimeOfDay = Object.entries(timeOfDayCounts)
        ...
        const mostCommonDifficulty = Object.entries(difficultyCounts)
            .sort((a, b) => b[1] - a[1])[0]?.[0] as TaskDifficulty || 'medium';
    ...
    private generateRecommendations(
        ...
        if (stats.averageErrorRate > 0.4) {
            recommendations.push({
                ...
                description: 'Your overall error rate suggests mental fatigue. Consider shorter, more focused sessions with breaks.',
                estimatedImpact: 40,
            });
        }
    ...
    async getArchiveStats(userId: string): Promise<ArchiveStats> {
        ...
        const mostCommonCategory = Array.from(categoryCounts.entries())
            .sort((a, b) => b[1] - a[1])[0]?.[0] || 'vocabulary';
    ...
    private compareToLastWeek(
        ...
        const currentAvgRate = currentWeaknesses.reduce((sum, w) => sum + w.errorRate, 0) / currentWeaknesses.length || 0;
        const previousAvgRate = previousMap.topWeaknesses.reduce((sum, w) => sum + w.errorRate, 0) / previousMap.topWeaknesses.length || 0;
    ...
    private groupAttemptsByCategory(attempts: (TaskAttempt & { category?: TaskCategory })[]): Record<TaskCategory, number> {
