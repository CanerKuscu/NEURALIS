...
export const QUICK_PROMPTS: QuickPrompt[] = [
    { id: 'explain', emoji: '💡', label: 'Explain a concept', labelTr: 'Bir kavram açıkla', prompt: 'Bana şu kavramı basitçe açıklar mısın: ' },
    { id: 'quiz', emoji: '❓', label: 'Make me a quiz', labelTr: 'Bana quiz yap', prompt: 'Bana şu konuda 3 soruluk bir quiz hazırla: ' },
    { id: 'compare', emoji: '⚖️', label: 'Compare two things', labelTr: 'İki şeyi karşılaştır', prompt: 'Şunları karşılaştır: ' },
    { id: 'funfact', emoji: '🤯', label: 'Tell a fun fact', labelTr: 'İlginç bir bilgi söyle', prompt: 'Bana şu konuda ilginç bir bilgi söyle: ' },
    { id: 'motivation', emoji: '💪', label: 'Motivate me', labelTr: 'Beni motive et', prompt: 'Öğrenmeye devam etmem için beni motive et!' },
    { id: 'summary', emoji: '📝', label: 'Summarize a topic', labelTr: 'Konuyu özetle', prompt: 'Şu konuyu kısa ve öz özetle: ' },
    { id: 'analogy', emoji: '🔗', label: 'Give an analogy', labelTr: 'Benzetme yap', prompt: 'Şu kavramı günlük hayattan bir benzetmeyle açıkla: ' },
    { id: 'steps', emoji: '📋', label: 'Step by step', labelTr: 'Adım adım göster', prompt: 'Şunu adım adım nasıl yapacağımı göster: ' },
];
    ...
    async sendMessage(sessionId: string, userMessage: string): Promise<AIChatMessage> {
        ...
        try {
            const { data, error } = await supabase.functions.invoke('generate-lesson', {
                body: {
                    mode: 'chat',
                    messages: session.messages.filter(m => m.role !== 'system').slice(-10).map(m => ({
                        role: m.role,
                        content: m.content,
                    })),
