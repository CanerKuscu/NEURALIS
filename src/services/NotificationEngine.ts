...
class NotificationEngine {
    ...
    private async registerForPushNotifications(): Promise<void> {
        try {
            ...
            if (Constants.appOwnership === 'expo') {
                console.log('[NotificationEngine] Running inside Expo Go - skipping push token registration');
                return;
            }
            ...
            try {
                ...
                if (userId) {
                    await supabase.from('profiles').upsert({ id: userId, push_token: token.data, expo_push_token: token.data, updated_at: new Date().toISOString() });
    ...
    private async preloadSounds(): Promise<void> {
        try {
            await Promise.all(Object.entries(SOUND_FILES).map(async ([key, source]) => {
     ...
     */
    private async playNotificationSound(soundFile: SoundFileKey, decreasePitch?: boolean): Promise<void> {
        try {
     ...
     */
    private calculateSchedule(deadlineTimestamp: number, streakCount: number): NotificationSchedule[] {
        const now = Date.now();
     ...
     */
    private async scheduleNotification(item: NotificationSchedule, streakCount: number): Promise<void> {
        const now = Date.now();
        ...
        try {
            ...
            console.log(`[NotificationEngine] Scheduled: ${item.severity} at ${item.scheduledTime.toISOString()}`);
        } catch (error) {
    ...
    async sendImmediateNotification(
        ...
        const soundFile: SoundFileKey = severity === 'normal' ? '1.mp3' : severity === 'urgent' ? '2.mp3' : '3.mp3';
