...
class AnalyticsService {
    ...
    async recordActivity(data: { lessonsCompleted?: number; xpEarned?: number; correct?: number; total?: number; timeSpent?: number; category?: string }): Promise<void> {
        ...
        if (!todayStat) {
            todayStat = {
                date: today, lessonsCompleted: 0, xpEarned: 0,
                correctAnswers: 0, totalAnswers: 0, timeSpent: 0,
                streakDay: 0, categories: {},
            };
    ...
    async getWeeklySummary(): Promise<WeeklySummary> {
        ...
        const bestDay = weekStats.reduce((best, s) => s.xpEarned > (best?.xpEarned || 0) ? s : best, weekStats[0]);
    ...
    async getMonthlySummary(): Promise<MonthlySummary> {
        ...
        for (let i = 0; i < sorted.length; i++) {
            if (i === 0 || new Date(sorted[i].date).getTime() - new Date(sorted[i - 1].date).getTime() === 86400000) {
                currentStreak++;
            } else {
                currentStreak = 1;
            }
        ...
        const categoryBreakdown = Object.entries(catMap).map(([category, xp]) => ({
            category, xp, percentage: Math.round((xp / totalCatXp) * 100),
        })).sort((a, b) => b.xp - a.xp);
    ...
    async getInsights(): Promise<PerformanceInsight[]> {
        ...
        if (weekly.improvement > 10) {
            insights.push({ type: 'positive', emoji: '📈', text: `Great! ${weekly.improvement}% more XP than last week!`, textTr: `Harika! Geçen haftaya göre %${weekly.improvement} daha fazla XP kazandın!` });
        } else if (weekly.improvement < -10) {
            insights.push({ type: 'negative', emoji: '📉', text: `XP dropped ${Math.abs(weekly.improvement)}% from last week`, textTr: `XP'in geçen haftaya göre %${Math.abs(weekly.improvement)} düştü. Biraz daha çalışalım!` });
        }

        if (weekly.accuracy >= 90) {
            insights.push({ type: 'positive', emoji: '🎯', text: `${weekly.accuracy}% accuracy! Almost perfect!`, textTr: `%${weekly.accuracy} doğruluk! Neredeyse mükemmel!` });
        } else if (weekly.accuracy < 60) {
            insights.push({ type: 'negative', emoji: '🔄', text: `${weekly.accuracy}% accuracy. Review more!`, textTr: `%${weekly.accuracy} doğruluk. Tekrar derslerini artır!` });
        }

        if (weekly.totalLessons >= 7) {
            insights.push({ type: 'positive', emoji: '🔥', text: `${weekly.totalLessons} lessons this week! On fire!`, textTr: `Bu hafta ${weekly.totalLessons} ders! Ateş gibisin!` });
        }

        if (insights.length === 0) {
            insights.push({ type: 'neutral', emoji: '💡', text: 'Keep learning consistently!', textTr: 'Düzenli öğrenmeye devam et!' });
        }
