...
class LessonCacheService {
    ...
    async getCachedLesson(
        ...
        try {
            ...
            if (data && data.length > 0) {
                const cached = data[0];
                console.log(`✅ Cache HIT: "${cached.title}" (quality: ${cached.quality_score}, served: ${cached.usage_count}x)`);
    ...
    generatePersonalizedIntro(
        ...
        const streakMessages = streak > 0
            ? [
                i18n.t('cache.streak_msg', { count: streak, defaultValue: `🔥 ${streak} day streak! Keep it up!` }),
            ]
            : [];
    ...
    async getCacheStats(): Promise<{
        ...
        try {
            ...
            const avgQuality = data.reduce((sum, l) => sum + (l.quality_score || 0), 0) / (totalCached || 1);
