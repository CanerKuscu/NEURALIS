...
import type { LeagueTier, LeagueBracket, BracketParticipant, UserProfile } from '../types';
...
const LEAGUE_ZONES: LeagueZoneConfig = {
  promotionPercentage: 0.2, // Top 20% get promoted
  ...
  private async processParticipantResult(
    ...
    try {
      ...
      if (error) {
        ...
        if (isMissingTable) {
          console.warn(
            '[GlobalMeritService] Missing table detected; skipping league_results persistence.',
          );
        } else {
          throw error;
        }
