...
const NOTIFICATION_MESSAGES: Record<
  NotificationPhase,
  Record<SupportedLocale, NotificationMessage>
> = {
  safe: {
    en: {
...
class NotificationScheduler {
    ...
    try {
      ...
      try {
        ...
        if (error) {
          ...
          if (isMissingTable) {
            console.warn(
              '[NotificationScheduler] Missing table detected; skipping scheduled_notifications persistence.',
            );
          } else {
            throw error;
          }
        ...
        console.error(
          '[NotificationScheduler] Unexpected error during scheduled_notifications upsert:',
          err,
        );
      }
  ...
  async playNotificationAudio(
    ...
    try {
      ...
      const { sound } = await Audio.Sound.createAsync(audioAssets[audioFile], {
