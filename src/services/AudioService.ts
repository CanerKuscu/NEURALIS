...
import type { StreakState, AudioEffectType, AudioConfig } from '../types';
...
class AudioService {
  ...
  async playNotification(streakState: StreakState): Promise<void> {
    ...
    const soundKey =
      `notification_${streakState === 'neural_decay' ? 'decay' : streakState}` as AudioFileKey;
   ...
   */
  private async playSound(soundKey: AudioFileKey, streakState: StreakState): Promise<void> {
    // Audio files not bundled yet - skip silently
