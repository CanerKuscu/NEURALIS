...
import type { SynapseLink, UserProfile } from '../types';
import type { NotificationLocale } from './LocalizationService';
...
> = {
  en: {
    decay_warning:
      '⚠️ Your partner {{name}} is losing discipline and risking YOUR synapse link! Warn them NOW!',
    critical_warning:
      '🚨 CRITICAL! {{name}} has {{minutes}} minutes left! Your shared synapse is about to BREAK!',
    streak_death:
      '💀 {{name}} has FAILED. Your synapse link is BROKEN. Shame them or forgive them.',
  ...
  de: {
    decay_warning:
      '⚠️ Dein Partner {{name}} verliert die Disziplin und riskiert DEINE Synapsenverbindung! Warne ihn JETZT!',
    critical_warning:
      '🚨 KRITISCH! {{name}} hat noch {{minutes}} Minuten! Eure gemeinsame Synapse wird BRECHEN!',
    streak_death:
      '💀 {{name}} hat VERSAGT. Eure Synapsenverbindung ist GEBROCHEN. Beschäme oder vergib.',
    revival: '🌟 {{name}} ist zurück! Die Synapsenverbindung heilt. Willkommen zurück, Krieger.',
  },
  es: {
    decay_warning:
      '⚠️ ¡Tu compañero {{name}} está perdiendo disciplina y arriesgando TU enlace de sinapsis! ¡Adviértele AHORA!',
    critical_warning:
      '🚨 ¡CRÍTICO! ¡{{name}} tiene {{minutes}} minutos! ¡Su sinapsis compartida está a punto de ROMPERSE!',
    streak_death:
      '💀 {{name}} ha FALLADO. Su enlace de sinapsis está ROTO. Avergüénzalo o perdónalo.',
    revival:
      '🌟 ¡{{name}} ha vuelto! El enlace de sinapsis se está curando. Bienvenido de nuevo, guerrero.',
  },
  fr: {
    decay_warning:
      '⚠️ Ton partenaire {{name}} perd sa discipline et risque TON lien synaptique ! Avertis-le MAINTENANT !',
    critical_warning:
      '🚨 CRITIQUE ! {{name}} a {{minutes}} minutes ! Votre synapse partagée va SE BRISER !',
    streak_death:
      '💀 {{name}} a ÉCHOUÉ. Votre lien synaptique est BRISÉ. Fais-lui honte ou pardonne-lui.',
...
class SynapseGuiltService {
  ...
  async notifyPartners(
    ...
    const tasks = synapseLinks.map(async (link) => {
      ...
      if (
        partnerProfile.notificationsEnabled === false ||
        partnerProfile.accountStatus !== 'active'
      ) {
        console.log(
          `[SynapseGuiltService] Skipping partner notifications for ${partnerId} (notificationsEnabled=${partnerProfile.notificationsEnabled} accountStatus=${partnerProfile.accountStatus})`,
        );
        return;
      }
      ...
      if (partnerProfile.expoPushToken) {
        ...
        console.log(
          `[SynapseGuiltService] Partner ${partnerId} has no push token; created DB notification only.`,
        );
      }
      ...
      console.log(
        `[SynapseGuiltService] Notification sent and rate-limited for ${key} for ${NOTIFICATION_RATE_LIMIT_MS}ms`,
      );
      try {
        const t = setTimeout(() => {
   ...
   */
  private calculateUserStatus(user: UserProfile): 'healthy' | 'decay' | 'critical' | 'dead' {
  ...
  private async getUserProfile(userId: string): Promise<UserProfile | null> {
    ...
    return {
      ...
      notificationsEnabled:
        typeof data.notifications_enabled !== 'undefined' ? data.notifications_enabled : true,
   ...
   */
  private interpolateMessage(template: string, params: Record<string, unknown>): string {
    let result = template;
