...
class StreakService {
  ...
  async initializeStreak(userId: string): Promise<StreakData> {
    ...
    try {
      ...
      if (error) {
        ...
        if (isMissingTable) {
          console.warn(
            '[StreakService] Missing table detected; continuing without persisting streak.',
          );
        } else {
          throw error;
        }
  ...
  async killStreak(
    ...
    const penalizedStreak = currentStreak > 1 ? Math.max(1, Math.floor(currentStreak * 0.8)) : 0;
