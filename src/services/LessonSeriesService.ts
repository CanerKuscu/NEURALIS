...
class LessonSeriesService {
  ...
  async createSeries(request: CreateSeriesRequest): Promise<LessonSeries | null> {
    try {
      ...
      // Save to Supabase
      const { error: seriesError } = await supabase.from('lesson_series').insert({
        id: series.id,
  ...
  private generateFallbackLesson(
    ...
    const difficultyModifier =
      difficulty === 'beginner' ? 'basic' : difficulty === 'advanced' ? 'advanced' : 'intermediate';
