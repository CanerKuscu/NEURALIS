...
import type { SynapseLink, SynapseLinkRequest, SynapseLinkStatus, UserProfile } from '../types';
...
class SynapseService {
   ...
   */
  async sendLinkRequest(fromUserId: string, toUserId: string): Promise<SynapseLinkRequest | null> {
   ...
   */
  subscribeToActiveLink(userId: string, callback: (link: SynapseLink | null) => void): () => void {
  ...
  private parseLinkData(data: Record<string, unknown>): SynapseLink {
    ...
    return {
      ...
      userADisplayName:
        (data.userADisplayName as string) || (data['user_a_display_name'] as string),
      userAStreak: (data.userAStreak as number) || (data['user_a_streak'] as number) || 0,
      userALastActivity: toTs(
        (data.userALastActivity as any) ?? (data['user_a_last_activity'] as any),
      ),
      userBId: (data.userBId as string) || (data['user_b_id'] as string),
      userBDisplayName:
        (data.userBDisplayName as string) || (data['user_b_display_name'] as string),
      userBStreak: (data.userBStreak as number) || (data['user_b_streak'] as number) || 0,
      userBLastActivity: toTs(
        (data.userBLastActivity as any) ?? (data['user_b_last_activity'] as any),
      ),
      sharedStreak: (data.sharedStreak as number) || (data['shared_streak'] as number) || 0,
      sharedStreakStartedAt: toTs(
        (data.sharedStreakStartedAt as any) ?? (data['shared_streak_started_at'] as any),
      ),
      ...
      breakReason:
        (data.breakReason as SynapseLink['breakReason']) ||
        (data['break_reason'] as SynapseLink['breakReason']),
    };
  }
  ...
  private parseRequestData(data: Record<string, unknown>): SynapseLinkRequest {
    ...
    return {
      ...
      status:
        (data.status as SynapseLinkRequest['status']) ||
        (data['status'] as SynapseLinkRequest['status']),
    };
  }
