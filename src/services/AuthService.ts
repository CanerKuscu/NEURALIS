...
const calculatePasswordStrength = (password: string, errors: PasswordError[]): PasswordStrength => {
...
class AuthService {
  ...
  async signUp(data: SignUpData): Promise<AuthResult> {
    try {
      ...
      if (!passwordValidation.isValid) {
        const errorMessage = passwordValidation.errors.map(getPasswordErrorMessage).join('. ');
        return {
          success: false,
          error: {
            code: 'weak-password',
  ...
  async verifyOtp(
    email: string,
    code: string,
    type: 'signup' | 'email' | 'recovery' = 'email',
  ): Promise<AuthResult> {
    try {
      ...
      if (!supaUser) {
        return {
          success: false,
          error: { code: 'verify-failed', message: 'Verification failed', field: 'general' },
        };
      }
  ...
  async login(data: LoginData): Promise<AuthResult> {
    try {
      ...
      if (!supaUser) {
        return {
          success: false,
          error: { code: 'login-failed', message: 'Login failed', field: 'general' },
        };
      }
    ...
    try {
      ...
      try {
        ...
        if (otpError) {
          ...
          return this.handleAuthError({
            code: otpError.message || 'auth/error',
            message: otpError.message || String(otpError),
          } as AuthError);
        }
  ...
  async checkEmailVerification(): Promise<boolean> {
    ...
    try {
      ...
      const verified = Boolean(
        profile?.emailVerified || profile?.verificationStatus === 'verified',
      );
      if (verified) {
        try {
  ...
  async verifyPasswordResetOtp(
    ...
    try {
      ...
      if (updateError) {
        return this.handleAuthError({
          code: updateError?.message || 'auth/error',
  ...
  // DATABASE OPERATIONS
  // ─────────────────────────────────────────────────────────────────────────
  private async createUserDocument(userId: string, signUpData: SignUpData): Promise<UserProfile> {
    ...
    if (typeof (signUpData as any).avatarUrl !== 'undefined')
      userRow.avatar_url = (signUpData as any).avatarUrl;
    if (typeof (signUpData as any).premiumExpiresAt !== 'undefined')
      userRow.premium_expires_at = (signUpData as any).premiumExpiresAt;
    if (typeof (signUpData as any).linkedUserId !== 'undefined')
      userRow.linked_user_id = (signUpData as any).linkedUserId;
    if (typeof (signUpData as any).linkedUserName !== 'undefined')
      userRow.linked_user_name = (signUpData as any).linkedUserName;
    if (typeof (signUpData as any).synapseStreak !== 'undefined')
      userRow.synapse_streak = (signUpData as any).synapseStreak;
    ...
    if (error) {
      ...
      if (
        ...
        if (__DEV__)
          console.warn('[AuthService] Row-level security prevented creating profiles row.');
      ...
      if (isMissingColumn) {
        if (__DEV__)
          console.warn(
            '[AuthService] Detected missing column(s) in profiles table. Retrying with reduced payload.',
          );
        ...
        try {
          const { data: minimalUpsert, error: minimalError } = await supabase
            .from('profiles')
            .upsert(minimalRow)
            .select()
            .maybeSingle();
  ...
  private async updateLastLogin(uid: string): Promise<void> {
    ...
    await supabase
      .from('profiles')
      .update({ last_login_at: nowIso, last_activity_at: nowIso, updated_at: nowIso })
      .eq('id', uid);
  }

  private async updateEmailVerificationStatus(uid: string, verified: boolean): Promise<void> {
    await supabase
      .from('profiles')
      .update({
