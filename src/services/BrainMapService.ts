...
class BrainMapService {
  ...
  async getBrainMap(userId: string): Promise<BrainMapData> {
    ...
    const skills: SkillNode[] = SKILL_DEFINITIONS.map((def) => {
      const catLevel = categoryLevels?.find(
        (c) =>
    ...
    return {
      ...
      skills,
      overallHealth,
      decayingCount: skills.filter((s) => s.healthState === 'decaying' || s.healthState === 'dead')
        .length,
      thrivingCount: skills.filter(
        (s) => s.healthState === 'thriving' || s.healthState === 'healthy',
      ).length,
