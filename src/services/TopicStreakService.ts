...
class TopicStreakService {
     ...
     */
    async getOrCreateTopic(userId: string, topic: string, category: string = 'custom'): Promise<TopicStreak | null> {
    ...
    didLessonToday(topic: TopicStreak): boolean {
        if (!topic.lastLessonAt) return false;
        return (
            this.toLocalDateKey(new Date(topic.lastLessonAt)) ===
            this.toLocalDateKey(new Date())
        );
    }
