...
class AllianceService {
     ...
     */
    async sendInvite(fromUserId: string, toUserId: string): Promise<{ success: boolean; message?: string }> {
        try {
            // Self-invite check
    ...
    async acceptInvite(allianceId: string, userId: string): Promise<boolean> {
        try {
            ...
            if (alliance) {
                await this.createNotification(alliance.user1_id, 'accepted', 'Your alliance invite was accepted! 🎉');
            }
     ...
     */
    async checkAllianceStreak(allianceId: string): Promise<{ bothCompleted: boolean; streak: number }> {
        try {
            const { data, error } = await supabase
                .from('alliances')
