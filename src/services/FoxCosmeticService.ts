...
class FoxCosmeticService {
  ...
  async purchaseCosmetic(
    ...
    if (cosmetic.isPremium) {
      return {
        success: false,
        message: 'Bu eşya sadece Premium üyelere özel!',
   ...
   */
  async equipCosmetic(userId: string, cosmeticId: string): Promise<FoxOutfit> {
    const cosmetic = getCosmeticById(cosmeticId);
   ...
   */
  async unequipCosmetic(userId: string, category: CosmeticCategory): Promise<FoxOutfit> {
    const outfit = await this.getActiveOutfit(userId);
