...
class AITutorService {
  ...
  async generateLesson(
    ...
    exercises.push(
      this.makeExerciseForConcept(weakConcepts[0] || 'general-vocab', difficulty, false),
    );
  ...
  async generateLessonFromDeepseek(userId?: string): Promise<Lesson | null> {
    try {
      ...
      console.warn(
        '[AITutorService] supabase.functions.invoke not available; Deepseek call skipped.',
      );
      return null;
    } catch (err) {
   ...
   */
  async generateWeeklyErrorReport(
    userId?: string,
  ): Promise<{ summary: Record<string, number>; recentErrors: any[] } | null> {
    try {
      if (!userId) {
        try {
          const sessionRes = await supabase.auth.getSession();
  ...
  private makeExerciseForConcept(
    concept: string,
    difficulty: 'easy' | 'medium' | 'hard',
    premiumOnly = false,
  ): Exercise {
  ...
  ): { accuracy: number; mistakesByConcept: Record<string, number> } {
    let correct = 0;
    let total = 0;
   ...
   */
  private parseAIResponse(response: string, task: Task, attempt: TaskAttempt): NeuralPathAnalysis {
    try {
  ...
  setAPIKey(_provider: AIProvider, _key: string): void {
    if (__DEV__)
      console.warn('[AITutorService] setAPIKey is deprecated — keys are managed server-side.');
  }
}
